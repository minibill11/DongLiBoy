...
namespace JianHeMES.Controllers
{
    public class Personnel_RosterController : Controller
    {
        ...
        public ActionResult LeavedPersonServiceLength(int year, int month)
        {
            ...
            var departmentlist = new List<string> { "MC部", "PC部", "SMT部", "技术部", "配套加工部", "品质部", "装配一部", "装配二部" };
            ...
            foreach (var item in departmentlist)
            {
                ...
                for (int i = 0; i < lessthan.Count; i++)
                {
                    if (lessthan[i] == 1)
                    {
                        var craftsman_DP_month = AllRecordList.Count(c => c.Department == item && c.levelType == "匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i]);
                        var not_craftsman_DP_month = AllRecordList.Count(c => c.Department == item && c.levelType == "非匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i]);
                    ...
                    {
                        var craftsman_DP_month3 = AllRecordList.Count(c => c.Department == item && c.levelType == "匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i]);
                        var not_craftsman_DP_month3 = AllRecordList.Count(c => c.Department == item && c.levelType == "非匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i]);
                    ...
                    var craftsman_DP_month2 = AllRecordList.Count(c => c.Department == item && c.levelType == "匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i] && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i + 1]);
                    var not_craftsman_DP_month2 = AllRecordList.Count(c => c.Department == item && c.levelType == "非匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i] && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i + 1]);
                ...
                int countByDepartment = Year_Month_Allrecord.Count(c => c.Department == item);
                //月初人数
                var begin_day_of_month = countByDepartment == 0 ? 0 : Year_Month_Allrecord.Where(c => c.Department == item).OrderBy(c => c.Date).FirstOrDefault().Employees_personnel + Year_Month_Allrecord.Where(c => c.Department == item).OrderBy(c => c.Date).FirstOrDefault().Today_on_board_employees;
                //月末人数
                var end_day_of_month = countByDepartment == 0 ? 0 : Year_Month_Allrecord.Where(c => c.Department == item).OrderByDescending(c => c.Date).FirstOrDefault().Employees_personnel + Year_Month_Allrecord.Where(c => c.Department == item).OrderByDescending(c => c.Date).FirstOrDefault().Today_on_board_employees;
                //部门实际人数(月初+月末平均人数)
                ...
                table1.Add(item, JsonConvert.SerializeObject(record1));
                table2.Add(item + "," + "匠师傅", JsonConvert.SerializeObject(record2));
                table2.Add(item + "," + "非匠师傅", JsonConvert.SerializeObject(record3));
            ...
            //表2：总计、匠师傅总计、非匠师傅总计、匠师傅占比、非匠师傅占比
            ...
            for (int i = 0; i < lessthan.Count; i++)
            {
                if (lessthan[i] == 1)
                {
                    ...
                    //表2
                    craftsmanSum.Add(AllRecordList.Count(c => c.levelType == "匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i]));
                    not_craftsmanSum.Add(AllRecordList.Count(c => c.levelType == "非匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i]));
                    craftsmanPercent.Add((Convert.ToDecimal(AllRecordList.Count(c => c.levelType == "匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i])) * 100 / total).ToString("#0.00") + "%");
                    not_craftsmanPercent.Add((Convert.ToDecimal(AllRecordList.Count(c => c.levelType == "非匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i])) * 100 / total).ToString("#0.00") + "%");
                }
                else if (lessthan[i] == 120)
                {
                    //表1
                ...
                //表2
                craftsmanSum.Add(AllRecordList.Count(c => c.levelType == "匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i] && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i + 1]));
                not_craftsmanSum.Add(AllRecordList.Count(c => c.levelType == "非匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i] && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i + 1]));
                craftsmanPercent.Add((Convert.ToDecimal(AllRecordList.Count(c => c.levelType == "匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i] && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i + 1])) * 100 / total).ToString("#0.00") + "%");
                not_craftsmanPercent.Add((Convert.ToDecimal(AllRecordList.Count(c => c.levelType == "非匠师傅" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i] && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i + 1])) * 100 / total).ToString("#0.00") + "%");

            }
            //匠师傅总计
            var craftsmanTotalcount = AllRecordList.Count(c => c.levelType == "匠师傅");
            //非匠师傅总计
            var not_craftsmanTotalcount = AllRecordList.Count(c => c.levelType == "非匠师傅");
            //匠师傅总占比
            ...
            table2.Add("匠师傅总计,匠师傅总计", JsonConvert.SerializeObject(craftsmanSum));

            table2.Add("非匠师傅总计,非匠师傅总计", JsonConvert.SerializeObject(not_craftsmanSum));
            table2.Add("匠师傅占比,匠师傅占比", JsonConvert.SerializeObject(craftsmanPercent));
            table2.Add("非匠师傅占比,非匠师傅占比", JsonConvert.SerializeObject(not_craftsmanPercent));
        ...
        public async Task<ActionResult> Index()
        {
            ...
            count.Add("匠师傅", await db.Personnel_Roster.CountAsync(c => c.levelType == "匠师傅"));
            count.Add("非匠师傅", await db.Personnel_Roster.CountAsync(c => c.levelType == "非匠师傅"));
            ...
            foreach (var dp in dplist)
            {
                ...
                {
                    detail.Add(dg==null?"空组":dg, await db.Personnel_Roster.CountAsync(c => c.Department == dp && c.DP_Group == dg));
                }
                ...
                {
                    detail.Add(ed==null?"空学历":ed, await db.Personnel_Roster.CountAsync(c => c.Department == dp && c.Education == ed));
                }
        ...
        public async Task<ActionResult> Index(string jobNum, string name, string status, string department)
        {
            ...
            count.Add("匠师傅", db.Personnel_Roster.Count(c => c.levelType == "匠师傅"));
            count.Add("非匠师傅", db.Personnel_Roster.Count(c => c.levelType == "非匠师傅"));
            result.Add("总统计", count);
