  ...
  <div>
    ...
    <el-main class="main-box">
      ...
      <div class="equipment-index-inputcontainer">
        ...
        <el-button id="search" type="primary" size="mini" @click="initData"
          >查询</el-button
        >
        <el-button
          type="primary"
          size="mini"
          @click="showCreateCheck = true"
          plain
          >创建点检表</el-button
        >
        <el-button type="primary" size="mini" @click="onShowCopy" plain
          >复制月数据</el-button
        >
        <el-button type="primary" size="mini" @click="onShowDayCheck" plain
          >查看日点检情况</el-button
        >
      </div>
      <div class="tip-box">
        <span class="tip"
          >提示：查询条件中部门名称（因组织架构变更）可能与当前组织架构的部门名称不一致。</span
        >
      </div>
      <!-- @* 点检表 *@ -->
      <div class="bottomContainer">
        ...
        >
          ...
          <el-table-column prop="UserDepartment" label="部门">
          </el-table-column>
          <el-table-column prop="LineName" sortable label="线别">
          </el-table-column>
      ...
      <!-- @* 创建点检表弹框 *@ -->
      <el-dialog title="创建点检表" :visible.sync="showCreateCheck" width="20%">
        <el-form label-width="100px" :model="addCheck">
          <el-form-item label="设备编号：">
            <el-select
              v-model="addCheck.EquipmentNumber"
              size="mini"
              allow-create
              filterable
              clearable
          ...
          <el-form-item label="部门：">
            <el-select
              v-model="addCheck.UserDepartment"
              size="mini"
              allow-create
              filterable
              clearable
        ...
        <span slot="footer" class="dialog-footer">
          <el-button size="small" @click="showCreateCheck = false"
            >取 消</el-button
          >
          <el-button size="small" type="primary" @click="onCreateCheck"
            >确 定</el-button
          >
        </span>
      </el-dialog>
      <!-- @* 修改使用部门弹框 *@ -->
      <el-dialog title="修改设备信息" :visible.sync="showMsg" width="25%">
        <el-form label-width="100px" :model="eqMsg">
          ...
          <el-form-item label="部门：">
            <el-select
              v-model="eqMsg.UserDepartment"
              size="mini"
              filterable
              placeholder="请选择使用部门"
              style="width: 150px"
              @change="onChangeUserDepartment"
            >
              <el-option
                v-for="item in deparlist"
                :key="item.value"
        ...
        <span slot="footer" class="dialog-footer">
          <el-button size="small" @click="showMsg = false">取 消</el-button>
          <el-button size="small" type="primary" @click="onModifyEquipment"
            >确 定</el-button
          >
        </span>
      </el-dialog>
      ...
      <el-dialog
        title="复制设备点检保养月记录"
        :visible.sync="copyMonthDialogVisible"
        width="35%"
      >
      ...
      <el-dialog
        title="今天点检情况"
        :visible.sync="dayCheckDialogVisible"
        width="36%"
      >
        <el-select
          ...
          <el-col :span="12">
            <el-card shadow="never" class="card-never">
              <div slot="header" class="clearfix">
                <span
                  >{{ select_department }}今天未点检设备数：{{ TallyNum }}</span
                >
              </div>
              <div class="text-box">
                <div
...
export default {
  ...
  methods: {
    ...
    onCreateCheck() {
      ...
      if (
        this.addCheck.equipmentNumber == "" ||
        this.addCheck.EquipmentName == "" ||
        this.addCheck.time == ""
      ) {
        this.$message.warning("请补全信息！");
      } else {
...
/deep/.card-have .el-card__header {
  color: @success-color;
...
/deep/.card-never .el-card__header {
  color: @error-color;
