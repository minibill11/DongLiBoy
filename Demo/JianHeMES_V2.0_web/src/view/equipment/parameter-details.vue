...
<template>
  <div>
    ...
    <el-main class="main-box">
      <div class="btn-box">
        <span>
          ...
          <el-button
            ...
            size="mini"
            plain
            v-on:click="onToTurn(1, '查看安全库存清单')"
            >安全库存清单</el-button
          >
        </span>
        <el-button type="primary" size="mini" v-on:click="onEdit">{{
          isEdit ? "保存" : "编辑"
        }}</el-button>
      </div>
          ...
          <tr>
            <th colspan="7">
              <div class="table-title">
                '{{ tableTopData.UserDepartment }}' 设备保养、维修履历资料汇总表
              </div>
            </th>
          </tr>
        </thead>
          ...
          <tr>
            ...
            <td>
              <el-select
                v-if="isEdit"
                size="mini"
                v-model="tableTopData.UserDepartment"
                placeholder="请选择..."
              >
                <el-option
          ...
          <tr>
            ...
            <td>
              <el-date-picker
                v-if="isEdit"
                size="mini"
                v-model="tableTopData.PurchaseDate"
                type="date"
                placeholder="选择日期..."
            ...
            <td>
              <el-date-picker
                v-if="isEdit"
                size="mini"
                v-model="tableTopData.ActionDate"
                type="date"
                placeholder="选择日期..."
                value-format="yyyy-MM-dd HH:mm:ss"
              >
            ...
            <th colspan="7">
              <div class="table-center-box">
                <div class="table-center-left">
                  ...
                  <template v-if="isMoreAdd">
                    ...
                    <el-button
                      ...
                      size="mini"
                      plain
                      v-on:click="onCancelMaintainMore"
                      >取消</el-button
                    >
                  </template>
                </div>
                ...
                <div class="table-center-right">
                  ...
                  <!-- 上传图片 -->
                  <ParameterDetailUpload
                    :equipmentNumber="tableTopData.EquipmentNumber"
                  ></ParameterDetailUpload>
                </div>
              </div>
            </th>
      ...
      <el-dialog
        title="预览大图"
        v-bind:visible.sync="showVisible"
        width="60%"
        style="text-align: left"
      >
        <img style="width: 100%" :src="dialogImageUrl" alt="" /> </el-dialog
      > 
      <el-table
      ...
      >
        <el-table-column label="序号" width="50">
          <template scope="scope">
            <span v-if="scope.row.color == undefined"
              >{{ scope.$index + 1 }}
            </span>
            <span
              v-if="scope.row.color != undefined"
              :class="scope.row.color == 1 ? 'textRed' : ''"
              >{{ scope.$index + 1 }}
            </span>
          </template>
        </el-table-column>
        <el-table-column width="100" label="报修人">
          <template slot-scope="scope">
            <div v-if="scope.row.ReportRepairMan != 'null'">
              {{ scope.row.ReportRepairMan }}
            </div>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="Reason" label="原因分析" width="180">
        </el-table-column>
        <el-table-column prop="RepairOrTestContent" label="维修或检测内容">
        </el-table-column>
        ...
        >
          <template slot-scope="scope">
            <div>{{ scope.row.GetJobTime | formatMinutes }}</div>
          </template>
        </el-table-column>
        <el-table-column
          prop="PlanFinishTime"
        ...
        >
          <template slot-scope="scope">
            <div>{{ scope.row.PlanFinishTime | formatMinutes }}</div>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <div>{{ scope.row.ActualFinishTime | formatMinutes }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="RepairMan" width="100" label="维修人">
        </el-table-column>
        ...
        <el-table-column label="操作" width="100">
          <template slot-scope="scope">
            <el-button
              ...
              type="text"
              size="mini"
              v-on:click="onToMaintain('1', scope.row)"
            ...
            <el-button
              ...
              type="text"
              size="mini"
              v-on:click="onDeleteMaintain(scope.$index)"
              style="text-decoration: underline"
              >删除</el-button
            >
          </template>
      ...
      >
          ...
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item label="状态开始时间：">
                <el-date-picker
                  size="mini"
                  v-model="formInfo.StatusStarTime"
                  type="date"
                  placeholder="请选择日期..."
                  value-format="yyyy-MM-dd HH:mm:ss"
                  clearable
            ...
            <el-col :span="6">
              <el-form-item label="状态结束时间：">
                <el-date-picker
                  size="mini"
                  v-model="formInfo.StatusEndTime"
                  type="date"
                  placeholder="请选择日期..."
                  value-format="yyyy-MM-dd HH:mm:ss"
                  clearable
          ...
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item label="计划完成时间：">
                <el-date-picker
                  size="mini"
                  v-model="formInfo.PlanFinishTime"
                  type="date"
                  placeholder="请选择日期..."
                  value-format="yyyy-MM-dd HH:mm:ss"
                  clearable
            ...
            <el-col :span="6">
              <el-form-item label="实际完成时间：">
                <el-date-picker
                  size="mini"
                  v-model="formInfo.ActualFinishTime"
                  type="date"
                  placeholder="请选择日期..."
                  value-format="yyyy-MM-dd HH:mm:ss"
                  clearable
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button size="mini" type="primary" v-on:click="RepiaInfo"
            >保存</el-button
          >
        </span>
      </el-dialog>
...
import ParameterDetailUpload from "./page-components/_parameter_detail_upload";
import {
  Equipment_Timeusage,
  Parameter_Details,
  Edit_Equipmentbasic,
  BatchAdd_Rrecord,
  Userdepartment_list,
  AddEquipmentStatusRecordAsync,
  EditEquipmentStatusRecordAsync,
} from "@/api/equipment";
export default {
  ...
  data() {
    return {
      ...
      department_options: [],
      //上传excel
      templateName: "设备模块_维修履历汇总批量上传模板.xls",
      url: "MES_Data/DocumentTemplateFiles/设备模块_维修履历汇总批量上传模板",
      tableData: [], //维修表数据
      //选择时间显示运行时间
  ...
  components: { DownloadTemplate, ImportExcel, ParameterDetailUpload },
  computed: {},
  watch: {
    select_date(val) {
  ...
  methods: {
    ...
    onToTurn(val, limit) {
      if (this.$limit(limit)) {
        if (val == 0) {
          window.open(
            "/Equipment/Equipment_Keycomponents?equipmenNum=" +
              this.tableTopData.EquipmentNumber
          );
        } else if (val == 1) {
          window.open(
            "/Equipment/Equipment_Safety_Bill?equipmentName=" +
              this.tableTopData.EquipmentName
          );
        }
      } else {
    ...
    getTimeList(val) {
      ...
      Equipment_Timeusage(param)
        .then((res) => {
          //console.log(res.data.Data[0].timeList);
          this.timeList = res.data.Data[0].timeList;
        })
        .catch((err) => {
          console.log(err);
        });
    ...
    // 获取初始表格数据
    getDefaultInfos() {
      let equipmentNumber = this.$route.query.num;
      Parameter_Details({ equipmentNumber: equipmentNumber })
        .then((res) => {
          //console.log(res.data.Data);
          this.tableTopData = JSON.parse(res.data.Data.basicinfo).Result;
          this.tableData = JSON.parse(res.data.Data.statusrecord).Result;
          // 获取图片
          if (res.data.Data.picture != "未上传图片。") {
            let img = JSON.parse(res.data.Data.picture);
            // console.log(img)
            img.forEach((item) => {
              item.FullPath = this.$loadPath + item.FullPath.replace(/D:/, "");
            });
            //console.log(img)
            this.imgArr = img;
          }
    ...
    onSave() {
      let param = {
        ...
        userdepartment: this.tableTopData.UserDepartment,
        status: this.tableTopData.Status,
      ...
      //console.log(param);
      Edit_Equipmentbasic(param).then((res) => {
        //console.log(res.data.Data);
        if (res.data.Message == "修改失败！") {
      ...
      if (response.length != 0) {
        ...
        json.forEach((item) => {
          let obj = {
            ...
            Section: this.tableTopData.Section,
            Modifier: this.tableTopData.Modifier,
            ReportRepairMan: item["报修人"],
            FailureDescription: item["故障现象描述"],
            Reason: item["原因分析"],
            RepairOrTestContent: item["维修或检测内容"],
            GetJobTime: item["维修接单时间"],
            PlanFinishTime: item["计划完成时间"],
            ActualFinishTime: item["实际完成时间"],
            RepairMan: item["维修人"],
            Remark: item["备注"],
            color: 0,
          };
        ...
        BatchAdd_Rrecord({
          ...
          .then((res) => {
            ...
            if (res.data.Result) {
              ...
              arr.forEach((item) => {
                this.tableData.forEach((j, index) => {
                  if (new Date(item) == new Date(j.GetJobTime)) {
                    console.log(111);
                    this.tableData[index].color = 1;
                  }
                });
              });
              console.log(this.tableData);
    ...
    RepiaInfo() {
      let recordMethod;
      if (this.isAdd) {
        recordMethod = AddEquipmentStatusRecordAsync(this.formInfo);
      } else {
        recordMethod = EditEquipmentStatusRecordAsync(this.formInfo);
      }
      recordMethod
        .then((res) => {
          if (res.data.Result) {
