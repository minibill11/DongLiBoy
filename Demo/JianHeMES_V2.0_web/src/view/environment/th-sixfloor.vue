...
export default {
  ...
  mounted() {
    ...
    var cn = $.hubConnection("http://172.16.6.145/Hub6");
    this.proxy = cn.createHubProxy("Hub6");
    this.proxy.on("sendTH6", (TH6_json) => {
      this.data = TH6_json;
      console.log(TH6_json); //打印接收结果
    });
    cn.start({ jsonp: true }).done(console.log("Hub6已连通"));
  },
  methods: {},
  created() {
    // let thisVue = this;
