...
export default {
  ...
  mounted() {
    ...
    var cn = $.hubConnection("http://172.16.6.145/Hub1");
    this.proxy = cn.createHubProxy("Hub1");
    this.proxy.on("sendTH1", (TH1_json) => {
      this.data = TH1_json;
      console.log(TH1_json); //打印接收结果
    });
    cn.start({ jsonp: true }).done(console.log("Hub1已连通"));
  },
  methods: {},
  created() {
    // let thisVue = this;
