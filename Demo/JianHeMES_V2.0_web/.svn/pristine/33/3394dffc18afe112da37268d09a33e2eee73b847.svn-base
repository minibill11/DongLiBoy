...
export default {
  ...
  mounted() {
    ...
    var cn = $.hubConnection("http://172.16.6.145/Hub3");
    this.proxy = cn.createHubProxy("Hub3");
    this.proxy.on("sendTH3", (TH3_json) => {
      this.data = TH3_json;
      console.log(TH3_json); //打印接收结果
    });
    cn.start({ jsonp: true }).done(console.log("Hub3已连通"));
  },
  methods: {},
  created() {
    // let thisVue = this;
