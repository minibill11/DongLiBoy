...
namespace JianHeMESEntities.Hubs
{
    ...
    public class ModuleProductionControlIndexBroadcaster
    {
        ...
        public JArray QueryExcution()
        {
            ...
            DateTime date = new DateTime(2020, 7, 1);
            List<OrderMgm> OrderList_All = db.OrderMgm.Where(c => c.Models != 0 && c.PlanInputTime >= date).ToList();//.OrderBy(c => c.BarCodeCreated)
            ...
            //    OrderList_All = OrderList_All.Where(c => !jsonarray.Contains(c.OrderNum)).ToList();
            //}
            ...
            foreach (var item in OutputOrderList)
            {
                ...
                var orderjson = array.Where(c => c["OrderNum"].ToString() == item.OrderNum).FirstOrDefault();
                #region SMT
                #region---------------------SMT首样部分
                ...
                var smtinfo = board.Where(c => c.Section == "SMT" && c.OrderNum == item.OrderNum).Select(c => c.UpdateDate).FirstOrDefault();

                if (smtinfo == null || (time - smtinfo).Value.TotalSeconds <= span|| orderjson == null)
                {
                    var ModelNum = 0;
                    var NormalNumSum = 0;
                ...
                else if (array.Count != 0)
                {
                    ...
                    OrderNum.Add("SmtArray", orderjson["SmtArray"]);
                ...
                #region AI
                var AIinfo = board.Where(c => c.Section == "AI" && c.OrderNum == item.OrderNum).Select(c => c.UpdateDate).FirstOrDefault();
                if (AIinfo == null || (time - AIinfo).Value.TotalSeconds <= span|| orderjson == null)
                {
                    ...
                    foreach (var lineitem in line)
                    {
                        ...
                        obj.Add("CompleteRate", Math.Round((double)info.Count * 100 / item.Models, 2) + "%");
                        obj.Add("CompleteInfo", info.Count + "/" + item.Models);
                        obj.Add("PassRate", Math.Round((double)info.Count(c => c == true) * 100 / item.Models, 2) + "%");
                ...
                #region  后焊
                var afterinfo = board.Where(c => c.Section == "后焊" && c.OrderNum == item.OrderNum).Select(c => c.UpdateDate).FirstOrDefault();
                if (afterinfo == null || (time - afterinfo).Value.TotalSeconds <= span || orderjson == null)
                {
                    JArray itemArray = new JArray();
                    ...
                    foreach (var lineitem in line)
                    {
                        ...
                        obj.Add("PassRate", Math.Round((double)info.Count(c => c == true) * 100 / item.Models, 2) + "%");
                        obj.Add("PassInfo", info.Count(c => c == true) + "/" + item.Models);
                        obj.Add("SamplingRate", Math.Round((double)aftersamp * 100 / info.Count, 2) + "%");
                ...
                {
                    //var obj = array.Where(c => c["OrderNum"].ToString() == item.OrderNum).FirstOrDefault();
                    OrderNum.Add("After", orderjson["After"]);
                }
                else
                {
                    JArray temp = new JArray();
                ...
                #region  灌胶前电测
                var belowinfo = board.Where(c => c.Section == "灌胶前电检" && c.OrderNum == item.OrderNum).Select(c => c.UpdateDate).FirstOrDefault();
                if (belowinfo == null || (time - belowinfo).Value.TotalSeconds <= span || orderjson == null)
                {
                    JArray itemArray = new JArray();
                    ...
                    if (info1.Count != 0)
                    {
                        ...
                        JObject obj = new JObject();
                        obj.Add("Line", null);
                        obj.Add("CompleteRate", Math.Round((double)info1.Count * 100 / item.Models, 2) + "%");
                        obj.Add("CompleteInfo", info1.Count + "/" + item.Models);
                        obj.Add("PassRate", Math.Round((double)info1.Count(c => c == true) * 100 / item.Models, 2) + "%");
                ...
                else if (array.Count != 0)
                {
                    ...
                    OrderNum.Add("Before", orderjson["Before"]);
                }
                else
                {
                    JArray temp = new JArray();
                ...
                #region  灌胶后电测
                var blowinfo = board.Where(c => c.Section == "模块电检" && c.OrderNum == item.OrderNum).Select(c => c.UpdateDate).FirstOrDefault();
                if (blowinfo == null || (time - blowinfo).Value.TotalSeconds <= span || orderjson == null)
                {
                    JArray itemArray = new JArray();
                    ...
                    if (info2.Count != 0)
                    {
                        ...
                        JObject obj = new JObject();
                        obj.Add("Line", null);
                        obj.Add("CompleteRate", Math.Round((double)info2.Count * 100 / item.Models, 2) + "%");
                        obj.Add("CompleteInfo", info2.Count + "/" + item.Models);
                        obj.Add("PassRate", Math.Round((double)info2.Count(c => c == true) * 100 / item.Models, 2) + "%");
                ...
                {
                   // var obj = array.Where(c => c["OrderNum"].ToString() == item.OrderNum).FirstOrDefault();
                    OrderNum.Add("Later", orderjson["Later"]);
                }
                else
                {
                    JArray temp = new JArray();
                ...
                #region  老化
                var burninfo = board.Where(c => c.Section == "老化" && c.OrderNum == item.OrderNum).Select(c => c.UpdateDate).FirstOrDefault();
                if (burninfo == null || (time - burninfo).Value.TotalSeconds <= span || orderjson == null)
                {
                    JArray itemArray = new JArray();
                    ...
                    if (info3.Count != 0)
                    {
                        JObject obj = new JObject();
                        obj.Add("Line", null);
                        obj.Add("CompleteRate", Math.Round((double)info3.Count * 100 / item.Models, 2) + "%");
                        obj.Add("CompleteInfo", info3.Count + "/" + item.Models);
                        obj.Add("PassRate", Math.Round((double)info3.Count(c => c == true) * 100 / item.Models, 2) + "%");
                ...
                {
                    //var obj = array.Where(c => c["OrderNum"].ToString() == item.OrderNum).FirstOrDefault();
                    OrderNum.Add("BurnIn", orderjson["BurnIn"]);
                }
                else
                {
                    JArray temp = new JArray();
                ...
                #region  外观电检
                var appinfo = board.Where(c => c.Section == "外观电检" && c.OrderNum == item.OrderNum).Select(c => c.UpdateDate).FirstOrDefault();
                if (appinfo == null || (time - appinfo).Value.TotalSeconds <= span || orderjson == null)
                {
                    JArray itemArray = new JArray();
                    ...
                    if (info4.Count != 0)
                    {
                        ...
                        JObject obj = new JObject();
                        obj.Add("Line", null);
                        obj.Add("CompleteRate", Math.Round((double)info4.Count * 100 / item.Models, 2) + "%");
                        obj.Add("CompleteInfo", info4.Count + "/" + item.Models);
                        obj.Add("PassRate", Math.Round((double)info4.Count(c => c == true) * 100 / item.Models, 2) + "%");
                ...
                {
                    //var obj = array.Where(c => c["OrderNum"].ToString() == item.OrderNum).FirstOrDefault();
                    OrderNum.Add("Appearance", orderjson["Appearance"]);
                }
                else
                {
                    JArray temp = new JArray();
                ...
                #region  内箱
                var insideinfo = board.Where(c => c.Section == "纸箱" && c.OrderNum == item.OrderNum).Select(c => c.UpdateDate).FirstOrDefault();
                if (insideinfo == null || (time - insideinfo).Value.TotalSeconds <= span || orderjson == null)
                {
                    JArray itemArray = new JArray();
                ...
                {
                    //var obj = array.Where(c => c["OrderNum"].ToString() == item.OrderNum).FirstOrDefault();
                    OrderNum.Add("InsideTheBox", orderjson["InsideTheBox"]);
                }
                else
                {
                    JArray temp = new JArray();
                ...
                #region  外箱
                var outsideinfo = board.Where(c => c.Section == "外箱" && c.OrderNum == item.OrderNum).Select(c => c.UpdateDate).FirstOrDefault();
                if (outsideinfo == null || (time - outsideinfo).Value.TotalSeconds <= span || orderjson == null)
                {
                    JArray itemArray = new JArray();
                ...
                {
                    //var obj = array.Where(c => c["OrderNum"].ToString() == item.OrderNum).FirstOrDefault();
                    OrderNum.Add("OutsideTheBox", orderjson["OutsideTheBox"]);
                }
                else
                {
                    JArray temp = new JArray();
                ...
                #region  出入库
                var warehouseinfo = board.Where(c => c.Section == "外箱" && c.OrderNum == item.OrderNum).Select(c => c.UpdateDate).FirstOrDefault();
                if (warehouseinfo == null || (time - warehouseinfo).Value.TotalSeconds <= span || orderjson == null)
                {
                    JArray itemArray = new JArray();
                ...
                {
                    //var obj = array.Where(c => c["OrderNum"].ToString() == item.OrderNum).FirstOrDefault();
                    OrderNum.Add("Warehouse", orderjson["Warehouse"]);
                }
                else
                {
                    JArray temp = new JArray();
            ...
            System.IO.File.WriteAllText(@"D:\MES_Data\TemDate\ModuleProductionController.json", output2);
