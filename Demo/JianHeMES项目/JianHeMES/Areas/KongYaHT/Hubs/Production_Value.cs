...
{ 
    ...
    public class Production_ValueBroadcaster
    {
        ...
        public JObject QueryExcution()
        {
            ...
            var productionOrder = db.Production_Value.Select(c => c.OrderNum).Distinct().ToList();
            int i = 0; 
            foreach (var item in productionOrder)
            {
                var warehouse = db.Warehouse_Join.Where(c => c.IsOut == false || c.WarehouseOutDate >= yesterday&&c.OrderNum==item).ToList();
