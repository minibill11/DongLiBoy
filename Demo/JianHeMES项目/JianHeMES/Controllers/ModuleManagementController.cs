...
namespace JianHeMES.Controllers
{
    public class ModuleManagementController : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult> Index(List<string> orderunm)
        {
            ...
            foreach (var item in totalOrderdum)
            {
                ...
                //内箱包装数量
                result.Add("innerPackCount", null);
                //外箱包装数量
                result.Add("outsidepackCount", null);
                //出入库情况
        ...
        public ActionResult GetValueFromOrderNum(string ordernum, string statue)
        {
            ...
            foreach (var item in packingList)//循环规则信息
            {
                ...
                if (statue == "内箱")
                {
                    count = db.ModuleInsideTheBox.Where(c => c.IsEmbezzle == false && c.IsMixture == false && c.Type == item.Type && c.OrderNum == item.OrderNum).Select(c => c.InnerBarcode).Distinct().Count(); ;//根据订单号和类型找 没有混装,没有挪用的 内箱包装打印记录
                }
                else
                {
        ...
        public ActionResult GetModuleInsideTheBoxInfo(string ordernum)
        {
            JObject result = new JObject();
            var totalCount = db.ModulePackageRule.Where(c => c.OrderNum == ordernum && c.Statue == "内箱").ToList();
            ...
            else
            {
                ...
                var princount = db.ModuleInsideTheBox.Where(c => c.OrderNum == ordernum).Select(c => c.InnerBarcode).Distinct().Count();
                var InnerBarcode = ordernum.Replace("-", "") + "-";
                JArray typearray = new JArray();
                totalCount.ForEach(c => typearray.Add(c.Type));
                for (var i = 1; i <= count; i++)
                {
                    var temp = InnerBarcode + i.ToString().PadLeft(3, '0');
        ...
        //创建内箱记录
        public ActionResult CreateModuleInsideTheBox(List<ModuleInsideTheBox> barcodeList)
        {
            JObject result = new JObject();
        ...
        //检验条码是否规范
        public ActionResult CheckBarcode(string ordernum, string barcode, bool hybrid = false)
        {
            JObject result = new JObject();
            var order = db.ModuleInsideTheBox.Where(c => c.InnerBarcode == barcode).Select(c => c.OrderNum).FirstOrDefault();//根据条码在条码表找订单
            ...
            //{
            //    result.Add("mes", "此条码已完成过内箱装箱确认");
            //    result.Add("pass", false);
            //}
            else
            {
                var Printingexit = db.ModuleOutsideTheBox.Where(c => c.InnerBarcode == barcode).Select(c => c.InnerBarcode).FirstOrDefault();//查找打印表里是否有相同的条码号
                if (Printingexit != null)//有就传fasle
                {
        ...
        /// <returns></returns>
        public string CreatePackangPrint(List<ModuleOutsideTheBox> printings, string ordernum, string Department1, string Group)
        {
            if (ModelState.IsValid)//判断数据格式是否正确
            {
                string error = "";
                ...
                else//代表没有重复打印的
                {
                    ...
                    printings.ForEach(c => { c.Department = Department1; c.Group = Group; });//保存数据
        ...
        //列表,根据订单号找到类型
        public ActionResult GetTypeList(string ordernum)
        {
            var orders = db.ModulePackageRule.Where(c => c.OrderNum == ordernum).Select(m => m.Type).Distinct().ToList();    //增加.Distinct()后会重新按OrderNum升序排序
        ...
        //列表,根据订单号,类型找到批号
        public ActionResult GetBatchList(string ordernum, string type)
        {
            var orders = db.ModulePackageRule.Where(c => c.OrderNum == ordernum && c.Type == type).Select(m => m.Batch).Distinct().ToList();    //增加.Distinct()后会重新按OrderNum升序排序
            JArray result = new JArray();
            foreach (var item in orders)
            {
                JObject List = new JObject();
        ...
        //列表,根据订单号,类型,批号找到屏号
        public ActionResult GetScreenList(string ordernum, string type, int batch)
        {
            var orders = db.ModulePackageRule.Where(c => c.OrderNum == ordernum && c.Type == type && c.Batch == batch).Select(m => m.ScreenNum).Distinct().ToList();    //增加.Distinct()后会重新按OrderNum升序排序
            JArray result = new JArray();
            foreach (var item in orders)
            {
                JObject List = new JObject();
