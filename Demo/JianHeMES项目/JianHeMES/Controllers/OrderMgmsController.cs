...
namespace JianHeMESEntities.Controllers
{
    public class OrderMgmsController : Controller
    {
        ...
        {
            var platformTypelist = db.OrderMgm.OrderByDescending(m => m.ID).Select(c => c.PlatformType).ToList();
            JArray result = new JArray();
            foreach (var item in platformTypelist)
            {
                JObject List = new JObject();
        ...
        public ActionResult Details(int? id)
        {
            ...
            if (Directory.Exists(@SMT_directory) == true)
            {
                ViewBag.SMT_Directory = "Exists";
            }
            //检查包装首件pdf文件是否存在
            string SMT_pdfFile = "D:\\MES_Data\\SMTSample_Files\\" + orderMgm.OrderNum + "\\" + orderMgm.OrderNum + "_SMTSample.pdf";
            if (System.IO.File.Exists(@SMT_pdfFile) == true)
            {
                ViewBag.SMT_PDf = "Exists";
            }
            List<FileInfo> SMT_filesInfo = new List<FileInfo>();
            JObject SMT_json = new JObject();
            if (ViewBag.SMT_Directory == "Exists")
            {
                SMT_filesInfo = GetAllFilesInDirectory(SMT_directory);
                SMT_filesInfo = SMT_filesInfo.Where(c => c.Name.StartsWith(orderMgm.OrderNum + "_SMTSample") && (c.Name.Substring(c.Name.Length - 4, 4) == ".jpg" || c.Name.Substring(c.Name.Length - 5, 5) == ".jpeg")).ToList();
                //检查包装首件jpg文件是否存在
                if (SMT_filesInfo.Count > 0)
                {
                    ViewBag.SMT_Picture = "Exists";
                }
                int i = 1;
                foreach (var item in SMT_filesInfo)
                {
                    SMT_json.Add(i.ToString(), item.Name);
                    i++;
                }
                ViewBag.SMT_jpgjson = SMT_json;
            }
            #endregion
            #endregion
        ...
        #region -------------------- 订单SMT首件
        [HttpPost]
        public ActionResult SMTSampleConvert(int? id, string SMT_Description)
        {
            if (Session["User"] == null)
            {
                return RedirectToAction("Login", "Users", new { col = "OrderMgms", act = "Details" + "/" + id.ToString() });
            ...
            orderMgm.IsSMTFirstSample = true;
            orderMgm.SMTFirstSampleDate = DateTime.Now;
            orderMgm.SMTFirstSampleConverter = ((Users)Session["User"]).UserName;
            orderMgm.SMTFirstSample_Description = SMT_Description;
            if (ModelState.IsValid)
            {
                db.Entry(orderMgm).State = EntityState.Modified;
                db.SaveChanges();
        ...
        #region --------------------上传SMT首件文件(jpg、pdf)方法
        [HttpPost]
        public ActionResult UploadSMTSampleFile(int id, string ordernum)
        {
            if (Request.Files.Count > 0)
            {
                HttpPostedFileBase file = Request.Files["uploadSMTSamplefile"];
                var index = file.FileName.LastIndexOf('.');
                var fileType = file.FileName.Substring(index, file.FileName.Length - index).ToLower();
                string ReName = ordernum + "_SMTSample";
                if (Directory.Exists(@"D:\MES_Data\SMTSample_Files\" + ordernum + "\\") == false)//如果不存在就创建订单文件夹
                {
                    Directory.CreateDirectory(@"D:\MES_Data\SMTSample_Files\" + ordernum + "\\");
                }
                List<FileInfo> fileInfos = GetAllFilesInDirectory(@"D:\MES_Data\SMTSample_Files\" + ordernum + "\\");
                //文件为jpg类型
                if (fileType == ".jpg")
                {
                    int jpg_count = fileInfos.Where(c => c.Name.StartsWith(ordernum + "_SMTSample") && c.Name.Substring(c.Name.Length - 4, 4) == fileType).Count();
                    file.SaveAs(@"D:\MES_Data\SMTSample_Files\" + ordernum + "\\" + ReName + (jpg_count + 1) + fileType);
                }
                else if (fileType == ".jpeg")
                {

                    int jpg_count = fileInfos.Where(c => c.Name.StartsWith(ordernum + "_SMTSample") && c.Name.Substring(c.Name.Length - 5, 5) == fileType).Count();
                    file.SaveAs(@"D:\MES_Data\SMTSample_Files\" + ordernum + "\\" + ReName + (jpg_count + 1) + fileType);
                ...
                {
                    file.SaveAs(@"D:\MES_Data\SMTSample_Files\" + ordernum + "\\" + ReName + fileType);
                }
                return RedirectToAction("Details", "OrderMgms", new { id = id });
            }
            return View();
        ...
        #region --------------------查看SMT首件图片预览
        [HttpPost]
        public ActionResult GetSMTSampleImg(string ordernum)
        {
            List<FileInfo> filesInfo = GetAllFilesInDirectory(@"D:\\MES_Data\\SMTSample_Files\\" + ordernum + "\\");
            filesInfo = filesInfo.Where(c => c.Name.StartsWith(ordernum + "_SMTSample") && (c.Name.Substring(c.Name.Length - 4, 4) == ".jpg" || c.Name.Substring(c.Name.Length - 5, 5) == ".jpeg")).ToList();
            JObject json = new JObject();
            int i = 1;
            if (filesInfo.Count() > 0)
            {
        ...
        #region --------------------查看SMT首件pdf文档页面
        public ActionResult preview_SMTSample_pdf(string ordernum)
        {
            List<FileInfo> filesInfo = new List<FileInfo>();
            string directory = "D:\\MES_Data\\SMTSample_Files\\" + ordernum + "\\";
            if (Directory.Exists(@directory) == false)//如果不存在就创建订单文件夹
            {
                return Content("PDF文档未上传或不存在！");
            }
            ...
            List<string> pdf_address = new List<string>();
            string address = "";
            if (filesInfo.Where(c => c.Name == ordernum + "_SMTSample.pdf").Count() > 0)
            {
                address = "/MES_Data/SMTSample_Files" + "/" + ordernum + "/" + ordernum + "_SMTSample.pdf";
            }
            else
            {
                return Content("PDF文档未上传或不存在！");
        ...
        public ActionResult unusualConvert(int? id, string unusualDescription, string statue)
        {
            ...
            switch (statue)
            {
                ...
                case "SMT首件":
                    ...
                    orderMgm.SMTFirstSampleDate = DateTime.Now;
                    orderMgm.SMTFirstSampleConverter = ((Users)Session["User"]).UserName;
                    orderMgm.SMTFirstSample_Description = unusualDescription;
                    break;
            }
            if (ModelState.IsValid)
            {
