    ...
    public class OrderMgmsController : Controller
    {
        ...
        public ActionResult Details(int? id)
        {
            ...
            if (ViewBag.AssembleAbnormalOrder_Directory == "Exists")
            {
                ...
                AssembleAbnormalOrder_filesInfo = AssembleAbnormalOrder_filesInfo.Where(c => c.Name.StartsWith(orderMgm.OrderNum) && c.Name.Substring(c.Name.Length - 4, 4) == ".jpg").ToList();
                //检查组装异常jpg文件是否存在
            ...
            if (ViewBag.BurnInAbnormalOrder_Directory == "Exists")
            {
                ...
                BurnInAbnormalOrder_filesInfo = BurnInAbnormalOrder_filesInfo.Where(c => c.Name.StartsWith(orderMgm.OrderNum) && c.Name.Substring(c.Name.Length - 4, 4) == ".jpg").ToList();
                //检查老化异常jpg文件是否存在
            ...
            if (ViewBag.AppearanceAbnormalOrder_Directory == "Exists")
            {
                ...
                AppearanceAbnormalOrder_filesInfo = AppearanceAbnormalOrder_filesInfo.Where(c => c.Name.StartsWith(orderMgm.OrderNum) && c.Name.Substring(c.Name.Length - 4, 4) == ".jpg").ToList();
                //检查包装异常jpg文件是否存在
        ...
        public ActionResult GetAssembleAbnormalOrderImg(string ordernum)
        {
            ...
            filesInfo = filesInfo.Where(c => c.Name.StartsWith(ordernum ) && c.Name.Substring(c.Name.Length - 4, 4) == ".jpg").ToList();
            JObject json = new JObject();
            int i = 1;
            foreach (var item in filesInfo)
            {
