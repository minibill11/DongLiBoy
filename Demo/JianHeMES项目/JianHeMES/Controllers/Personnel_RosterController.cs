        ...
        public ActionResult LeavedPersonServiceLength(int year, int month)
        {
            ...
            foreach (var item in departmentlist)
            {
                ...
                for (int i = 0; i < lessthan.Count; i++)
                {
                    ...
                    {
                        //操作族小于一个月
                        var craftsman_DP_month = AllRecordList.Count(c => c.Department == item && c.levelType == "操作族" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i]);
                        //非操作组小于一个月
                        var not_craftsman_DP_month = AllRecordList.Count(c => c.Department == item && c.levelType == "非操作族" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i]);
                        //部门小于一个月
                    ...
                    {
                        var craftsman_DP_month3 = AllRecordList.Count(c => c.Department == item && c.levelType == "操作族" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i]);
                        var not_craftsman_DP_month3 = AllRecordList.Count(c => c.Department == item && c.levelType == "非操作族" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i]);
                    ...
                    var craftsman_DP_month2 = AllRecordList.Count(c => c.Department == item && c.levelType == "操作族" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i] && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i + 1]);
                    var not_craftsman_DP_month2 = AllRecordList.Count(c => c.Department == item && c.levelType == "非操作族" && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) >= lessthan[i] && common.TwoDTforMonth_sub(c.LastDate, c.HireDate) < lessthan[i + 1]);
                ...
                //表2
                table2.Add(item + "," + "操作族", JsonConvert.SerializeObject(record2));
                table2.Add(item + "," + "非操作族", JsonConvert.SerializeObject(record3));
                table2.Add(item + "," + "小计", JsonConvert.SerializeObject(record1));
