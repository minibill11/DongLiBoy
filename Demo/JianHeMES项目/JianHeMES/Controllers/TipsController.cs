    ...
    public class TipsController : Controller
    {
        ...
        public ActionResult Index(int userId)
        {
            ...
            foreach (var item in userTipsItemList)
            {
                ...
                if (item.Matters == "设备月保养时间计划表确认审核") results.Add("设备月保养时间计划表确认审核", JsonConvert.SerializeObject(Equipment_MonthlyMainQuery()));
                if (item.Matters == "设备质量指标达成状况统计表审核批准") results.Add("设备质量指标达成状况统计表审核批准", JsonConvert.SerializeObject(Equipment_QualitytargetQuery()));
            }
        ...
        public int Pending_Work(int userId)
        {
            ...
            foreach (var item in userTipsItemList)
            {
                ...
                //月保养时间计划表
                if (item.Matters == "设备月保养时间计划表确认审核") count = count + Equipment_MonthlyMainQuery().Count;               
                //质量目标达成状况统计表
                if (item.Matters == "设备质量指标达成状况统计表审核批准）") count = count + Equipment_QualitytargetQuery().Count;
        ...
        public JArray Equipment_MonthlyMainQuery()
        {
            ...
            JObject plan2 = new JObject();
            JObject plan3 = new JObject();
            JObject plan4 = new JObject();

            DateTime dt = DateTime.Now;
            DateTime d1 = new DateTime(dt.Year, dt.Month, 1);
            DateTime d2 = d1.AddMonths(1).AddDays(-1);

            var tem = db.Equipment_MonthlyMaintenance.Where(c => c.Mainten_Lister != null && c.AssortDepar == null && dt == d2).ToList();
            ...
            var tem2 = db.Equipment_MonthlyMaintenance.Where(c => c.AssortDepar != null && c.Tec_Notarize == null).ToList();
            foreach (var main2 in tem2)//技术部确认
            {
                plan2.Add("UserDepartment", main2.UserDepartment);
                plan2.Add("year", main2.Year);
                plan2.Add("month", main2.Month);
                Timeplan.Add(plan2);
                plan2 = new JObject();
            }
            var tem3 = db.Equipment_MonthlyMaintenance.Where(c => c.Tec_Notarize != null && c.PCDepar == null).ToList();
            foreach (var main3 in tem3)//PC部确认
            {
                plan3.Add("UserDepartment", main3.UserDepartment);
                plan3.Add("year", main3.Year);
                plan3.Add("month", main3.Month);
                Timeplan.Add(plan3);
                plan3 = new JObject();
            }
            var tem4 = db.Equipment_MonthlyMaintenance.Where(c => c.PCDepar != null && c.Assessor == null).ToList();
            foreach (var main4 in tem4)//审核
            {
                plan4.Add("UserDepartment", main4.UserDepartment);
                plan4.Add("year", main4.Year);
                plan4.Add("month", main4.Month);
                Timeplan.Add(plan4);
                plan4 = new JObject();
            }
            return Timeplan;
        ...
        public JArray Equipment_QualitytargetQuery()
        {
            ...
            foreach (var item2 in quatar2)//审核
            {
                target2.Add("year", item2.Year);
                target2.Add("month", item2.Month);
                quality.Add(target2);
                target2 = new JObject();
            }
            return quality;
