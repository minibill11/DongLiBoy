...
namespace JianHeMES.Controllers
{
    public class CalibrationRecordsController : Controller
    {
        ...
        public ActionResult Batch_CalibrationRecord()
        {
            if (Session["User"] == null)
            {
                return RedirectToAction("Login2", "Users", new { col = "CalibrationRecords", act = "Batch_CalibrationRecord" });
            }
            return View();
        }
        #region --------------------模组校正首页
        ...
        public ActionResult FinishCal(int? id, bool sequence = true)
        {
            if (Session["User"] == null)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            calibrationRecord.ModuleGroupNum = json[index - 1].ToString();
                        }
                    }
                }
    ...
    public class CalibrationRecordsAPIController : System.Web.Http.ApiController
    {
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject BatchCreateCal([System.Web.Http.FromBody]JObject data)
        {
            //格式:{"BarCodesNumList":["xxx","xxxx","xxxx"]}
            ...
            foreach (var item in BarCodesNumList)
            {
                ...
                var BarCodesNum = item.ToString();
                var barcodeCheck = comapi.CheckBarcode(OrderNum, BarCodesNum);
                //检查订单和条码是否正确
                if (barcodeCheck != "true")
                {
                    resultitem.Add("Message", barcodeCheck);
                    resultitem.Add("Result", false);
                    resultitem.Add("id", null);
                    resultitem.Add("barcode", BarCodesNum);
                    resultitem.Add("ModuleGroupNum", null);
                    result.Add(resultitem);
                }
                else
                {
                    var ordernum = db.BarCodes.Where(c => c.BarCodesNum == BarCodesNum).FirstOrDefault();
                    string ModuleGroupNum = ordernum.ModuleGroupNum;
                    var modulelist = db.CalibrationRecord.Where(c => c.OrderNum == OrderNum && c.ModuleGroupNum != null).Select(c => c.ModuleGroupNum).ToList();
                    ...
                    string statue = comapi.CheckSectionStart(BarCodesNum, "CalibrationRecords", "BarCodesNum", "FinishCalibration", "Normal");
                    if (statue == "新增")
                    {
                        CalibrationRecord calibrationRecord = new CalibrationRecord();
                        calibrationRecord.OrderNum = OrderNum;
                        ...
                        resultitem.Add("Message", "成功");
                        resultitem.Add("Result", true);
                        resultitem.Add("id", calibrationRecord.ID);
                        resultitem.Add("barcode", BarCodesNum);
                        resultitem.Add("ModuleGroupNum", ModuleGroupNum);
                        result.Add(resultitem);
                    ...
                    {
                        resultitem.Add("Message", "该条码已经完成校正,要重复校正请单个重复校正");
                        resultitem.Add("Result", false);
                        resultitem.Add("id", null);
                        resultitem.Add("barcode", BarCodesNum);
                        resultitem.Add("ModuleGroupNum", ModuleGroupNum);
        ...
        public JObject FinishCal1([System.Web.Http.FromBody]JObject data)
        {
            ...
            {
                calibrationRecord.FinishCalibration = DateTime.Now;
                JObject time = comapi.CalculateTimespan(calibrationRecord.BeginCalibration.Value, calibrationRecord.FinishCalibration.Value);
                calibrationRecord.CalibrationTime = TimeSpan.Parse(time["Time"].ToString());
                calibrationRecord.CalibrationDate = int.Parse(time["Date"].ToString());
                calibrationRecord.CalibrationTimeSpan = time["TimeSpan"].ToString();
            }
            if (ModelState.IsValid)
            {
                //判断输入的模组号是否重复,并修改模组号
                JObject jobject = comapi.UpdateTotalModule(calibrationRecord.OrderNum, calibrationRecord.BarCodesNum, ModuleGroupNum, UserName);
                //如果修改成功
                if (jobject["pass"].ToString() == "True")
                {
                    ...
                    db.SaveChanges();
                    //如果有json文件,删除json文件
                    comapi.DeleteCaliJosn(calibrationRecord.OrderNum, ModuleGroupNum);
                    result.Add("mes", "校正成功");
                    result.Add("pass", true);
                    return com.GetModuleFromJobjet(result);
                }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject BatchFinishCal1([System.Web.Http.FromBody]JObject data)
        {
            //格式:{"value":[{"id":"1","AbnormalDescription":"正常","ModuleGroupNum":"123"},{"id":"2","AbnormalDescription":"异常信息","ModuleGroupNum":"456"}]}
            JArray result = new JArray();
            JArray value = (JArray)data["value"];
            string UserName = data["UserName"].ToString();
            foreach (var item in value)
            {
                JObject resultitem = new JObject();
                int id = int.Parse(item["id"].ToString());
                string AbnormalDescription = item["AbnormalDescription"].ToString();//正常或异常
                string ModuleGroupNum = item["ModuleGroupNum"].ToString();//模组号
                var calibrationRecord = db.CalibrationRecord.Find(id);
                ...
                if (jobject["pass"].ToString() == "True")
                {
                    ...
                    //如果有json文件,删除json文件
                    comapi.DeleteCaliJosn(calibrationRecord.OrderNum, ModuleGroupNum);
                    resultitem.Add("Message", "校正成功");
                    resultitem.Add("Result", true);
                    resultitem.Add("id", id);
                    result.Add(resultitem);
                }
                else
                {
                    resultitem.Add("Message", "校正失败");
                    resultitem.Add("Result", false);
                    resultitem.Add("id", id);
                    result.Add(resultitem);
                }
        ...
        public JObject CalibrationChecklist([System.Web.Http.FromBody] JObject data)
        {
            ...
            {
                //调出订单对应全部记录      
                var AllCalibrationRecord = db.CalibrationRecord.Where(c => c.OrderNum == orderNum && (c.OldOrderNum == null || c.OldOrderNum == orderNum)).Select(c => new Common_ApiController.Temp { id = c.ID, BarCodesNum = c.BarCodesNum, Finsh = c.Normal, FinshTime = c.FinishCalibration, ModuleGroupNum = c.ModuleGroupNum }).ToList();
