...
namespace JianHeMES.Controllers
{
    public class QueryController : Controller
    {
        ...
        public ActionResult DisplayBarcodeListFromOrdernum(string ordernum)
        {
            ...
            var warehouse = db.Warehouse_Join.Where(c => c.CartonOrderNum == ordernum && c.State == "模组").Select(c => new TempWarehouse { Barcode = c.BarCodeNum, IsOut = c.IsOut, id = c.Id }).ToList();
            #endregion
        ...
        public ActionResult SelectOrderInfoFromBusiness(string ordernum, bool alling = false)
        {
            ...
            var after = db.AfterWelding.Select(c => new TEMP { OrderNum = c.Ordernum, Finish = !c.IsAbnormal, EndTime = c.AfterWeldingTime, id = c.ID }).ToList();
            #endregion
    ...
    public class Query_ApiController : System.Web.Http.ApiController
    {
        private ApplicationDbContext db = new ApplicationDbContext();
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject DisplayBarcodeListFromOrdernum([System.Web.Http.FromBody]JObject data)
        {
            ...
            barcodeList.ForEach(c => { JObject item = new JObject(); item.Add("barcode", c); result.Add(item); });

            //拿到每个工段的最后一条数据,如果只有一条并且是正常完成的是1是正常完成;
            //如果有1条以上,最后一条是正常完成的是2是有异常完成;
            //没有记录是3是未开始;
            //只有一条并且没有完成时间4是正常进行中;
            //有1条以上记录并且最后一条没有完成时间或者,最后一条是异常完成的是5是异常进行中

            //组装
            var assemble = (from s in db.Assemble where s.OrderNum == ordernum && (s.OldOrderNum == ordernum || s.OldOrderNum == null) group s by s.BoxBarCode into g select new Temp { id = g.Max(c => c.Id), BoxBarCode = g.Key, count = g.Count(c => c.BoxBarCode == g.Key), FT = g.OrderByDescending(c => c.Id).Select(c => c.PQCCheckFT).FirstOrDefault(), Finsh = g.OrderByDescending(c => c.Id).Select(c => c.PQCCheckFinish).FirstOrDefault() }).ToList();
            result = BarcodeListFromOrdernumStaue(assemble, result, barcodeList, "Assemble");

            //FQC
            var finalqc = (from s in db.FinalQC where s.OrderNum == ordernum && (s.OldOrderNum == ordernum || s.OldOrderNum == null) group s by s.BarCodesNum into g select new Temp { id = g.Max(c => c.Id), BoxBarCode = g.Key, count = g.Count(c => c.BarCodesNum == g.Key), FT = g.OrderByDescending(c => c.Id).Select(c => c.FQCCheckFT).FirstOrDefault(), Finsh = g.OrderByDescending(c => c.Id).Select(c => c.FQCCheckFinish).FirstOrDefault() }).ToList();
            result = BarcodeListFromOrdernumStaue(finalqc, result, barcodeList, "FinalQC");

            //拼屏
            var mosaicscreen = db.Burn_in_MosaicScreen.Where(s => s.OrderNum == ordernum && (s.OldOrderNum == ordernum || s.OldOrderNum == null)).Select(c => c.BarCodesNum).Distinct().ToList();
            var barlist = barcodeList.Except(mosaicscreen).ToList();
            var d1 = result.Where(c => barlist.Contains(c["barcode"].ToString())).ToList();
            d1.ForEach(c => c["Burn_in_MosaicScreen"] = 3);
            var doing = mosaicscreen.Except(barlist).ToList();
            var burnindoing = db.Burn_in.Where(s => doing.Contains(s.BarCodesNum) && (doing.Contains(s.OldBarCodesNum) || s.OldBarCodesNum == null)).Select(c => c.BarCodesNum).ToList();
            var d2 = result.Where(c => burnindoing.Contains(c["barcode"].ToString())).ToList();
            d2.ForEach(c => c["Burn_in_MosaicScreen"] = 1);
            var notdoing = doing.Except(burnindoing).ToList();
            var d3 = result.Where(c => notdoing.Contains(c["barcode"].ToString())).ToList();
            d3.ForEach(c => c["Burn_in_MosaicScreen"] = 4);

            //老化
            var burnin = (from s in db.Burn_in where s.OrderNum == ordernum && (s.OldOrderNum == ordernum || s.OldOrderNum == null) group s by s.BarCodesNum into g select new Temp { id = g.Max(c => c.Id), BoxBarCode = g.Key, count = g.Count(c => c.BarCodesNum == g.Key), FT = g.OrderByDescending(c => c.Id).Select(c => c.OQCCheckFT).FirstOrDefault(), Finsh = g.OrderByDescending(c => c.Id).Select(c => c.OQCCheckFinish).FirstOrDefault() }).ToList();
            result = BarcodeListFromOrdernumStaue(burnin, result, barcodeList, "Burn_in");

            //校正
            var calibrationRecord = (from s in db.CalibrationRecord where s.OrderNum == ordernum && (s.OldOrderNum == ordernum || s.OldOrderNum == null) group s by s.BarCodesNum into g select new Temp { id = g.Max(c => c.ID), BoxBarCode = g.Key, count = g.Count(c => c.BarCodesNum == g.Key), FT = g.OrderByDescending(c => c.ID).Select(c => c.FinishCalibration).FirstOrDefault(), Finsh = g.OrderByDescending(c => c.ID).Select(c => c.Normal).FirstOrDefault() }).ToList();
            result = BarcodeListFromOrdernumStaue(calibrationRecord, result, barcodeList, "CalibrationRecord");


            //电检
            var appearn = (from s in db.Appearance where s.OrderNum == ordernum && (s.OldOrderNum == ordernum || s.OldOrderNum == null) group s by s.BarCodesNum into g select new Temp { id = g.Max(c => c.Id), BoxBarCode = g.Key, count = g.Count(c => c.BarCodesNum == g.Key), FT = g.OrderByDescending(c => c.Id).Select(c => c.OQCCheckFT).FirstOrDefault(), Finsh = g.OrderByDescending(c => c.Id).Select(c => c.OQCCheckFinish).FirstOrDefault() }).ToList();
            result = BarcodeListFromOrdernumStaue(appearn, result, barcodeList, "Appearance");

            //包装
            var packingList = db.Packing_BarCodePrinting.Where(c => c.CartonOrderNum == ordernum).Select(c => c.BarCodeNum).ToList();
            var d4 = result.Where(c => packingList.Contains(c["barcode"].ToString())).ToList();
            d4.ForEach(c => c["Packing"] = "已包装");
            var notpacking = barcodeList.Except(packingList).ToList();
            var d5 = result.Where(c => notpacking.Contains(c["barcode"].ToString())).ToList();
            d5.ForEach(c => c["Packing"] = "未包装");

            //出入库
            var warehousejoin = db.Warehouse_Join.Where(c => c.CartonOrderNum == ordernum && c.State == "模组").Select(c => c.BarCodeNum).ToList();
            var warehouseout = db.Warehouse_Join.Where(c => c.CartonOrderNum == ordernum && c.State == "模组" && c.IsOut == true).Select(c => c.BarCodeNum).ToList();
            var notjoin = barcodeList.Except(warehousejoin).ToList();
            var d6 = result.Where(c => notpacking.Contains(c["barcode"].ToString())).ToList();
            d6.ForEach(c => c["Warehoujoin"] = "未入库");
            var d7 = result.Where(c => warehousejoin.Contains(c["barcode"].ToString())).ToList();
            d7.ForEach(c => c["Warehoujoin"] = "已入库");

            var notout = warehousejoin.Except(warehouseout).ToList();
            var d9 = result.Where(c => warehousejoin.Contains(c["barcode"].ToString())).ToList();
            d9.ForEach(c => c["Warehouout"] = "未出库");

            var d10 = result.Where(c => warehouseout.Contains(c["barcode"].ToString())).ToList();
            d10.ForEach(c => c["Warehouout"] = "已出库");


            return com.GetModuleFromJarray(result);
        }

        public JArray BarcodeListFromOrdernumStaue(List<Temp> value, JArray barcodeList1, List<string> barcodeList, string key)
        {
            var assembleBarcodeList = value.Select(c => c.BoxBarCode).ToList();
            var assmeone = value.Where(c => c.Finsh == true && c.count == 1).Select(c => c.BoxBarCode).Distinct().ToList();
            var d1 = barcodeList1.Where(c => assmeone.Contains(c["barcode"].ToString())).ToList();
            d1.ForEach(c => c[key] = 1);
            var assmetwo = value.Where(c => c.Finsh == true && c.count > 1).Select(c => c.BoxBarCode).Distinct().ToList();
            var d2 = barcodeList1.Where(c => assmetwo.Contains(c["barcode"].ToString())).ToList();
            d2.ForEach(c => c[key] = 2);
            var assmethree = barcodeList.Except(assembleBarcodeList).ToList();
            var d3 = barcodeList1.Where(c => assmethree.Contains(c["barcode"].ToString())).ToList();
            d3.ForEach(c => c[key] = 3);
            var assmeflour = value.Where(c => c.FT == null && c.count == 1).Select(c => c.BoxBarCode).Distinct().ToList();
            var d4 = barcodeList1.Where(c => assmeflour.Contains(c["barcode"].ToString())).ToList();
            d4.ForEach(c => c[key] = 4);
            var assmefive = value.Where(c => (c.FT == null && c.count > 1) || (c.FT != null && c.Finsh == false)).Select(c => c.BoxBarCode).Distinct().ToList();
            var d5 = barcodeList1.Where(c => assmefive.Contains(c["barcode"].ToString())).ToList();
            d5.ForEach(c => c[key] = 5);
            return barcodeList1;
        }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject DisplayBarcodesModule([System.Web.Http.FromBody]JObject data)
        {
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject DisplayBarcodesOuther([System.Web.Http.FromBody]JObject data)
        {
            string barcode = data["barcode"].ToString();
            var outherBarcode = db.Packing_BarCodePrinting.Where(c => c.OuterBoxBarcode == barcode && c.QC_Operator == null).ToList();
            if (outherBarcode.Count != 0)
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject DisplayBarcodesSulder([System.Web.Http.FromBody]JObject data)
        {
            string barcode = data["barcode"].ToString();
            var sulderbarcode = db.Barcode_Solderpaste.Where(c => c.SolderpasterBacrcode == barcode).FirstOrDefault();
            if (sulderbarcode != null)
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject OrderNumReport([System.Web.Http.FromBody]JObject data)
        {
            string ordernum = data["ordernum"].ToString();
            JObject result = new JObject();
            #region 订单基本信息
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject SelectOrderInfoFromBusiness([System.Web.Http.FromBody]JObject data)
        {
            var datavlue = JsonConvert.DeserializeObject<dynamic>(JsonConvert.SerializeObject(data));
            string ordernum = datavlue.ordernum;
            string UserName = datavlue.UserName;
            bool alling = datavlue.alling ?? false;
            ...
            UserOperateLog log = new UserOperateLog() { Operator = UserName, OperateDT = DateTime.Now, OperateRecord = alling == true ? "在合约部或销售部选择了全部查询" : "在合约部或销售部查询了订单" + ordernum };
            db.UserOperateLog.Add(log);
            db.SaveChanges();
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject OrdernumList([System.Web.Http.FromBody]JObject data)
        {
