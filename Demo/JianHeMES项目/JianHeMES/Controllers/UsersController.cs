...
namespace JianHeMES.Controllers
{
    ...
    public class UsersController : Controller
    {
        ...
        public ActionResult LookLimits()
        {
            if (Session["User"] == null)
            {
                return RedirectToAction("Login", "Users", new { col = "Users", act = "LookLimits" });
            }
            return View();
        }

        #region----根据最新导航栏重写权限管理功能
        //显示模块
        ...
        //授权权限，取消权限
        public bool Authorization(string json)
        {
        ...
        public void Delete_Roles(int id, string rolesname)
        {
        ...
        public ActionResult PermissionList(string department, List<int> userID)
        {
            ...
            if (dataList.Count > 0)
            {
                ...
                foreach (var itg in idList)
                {
                    ...
                    {
                        if (ite.RolesName != null && ite.Roles != null)
                        {
        ...
        public ActionResult Login3(int UserNum, string PassWord, string col, string act)
        {
            JObject result = new JObject();
            var item = db.Users.FirstOrDefault(u => u.UserNum == UserNum && u.PassWord == PassWord);
            if (item != null)
            {
                if (item.Department == "销售部")
                {
        ...
        //[HttpGet]
        ////[Route("GetUserInfo")]
        //public ActionResult Login4(int UserNum, string PassWord, string col="", string act="")
        //{
        //    JObject result = new JObject();
        //    var item = db.Users.FirstOrDefault(u => u.UserNum == UserNum && u.PassWord == PassWord);
        //    if (item != null)
        ...
        //        result.Add("UserID", UserNum);
        //        result.Add("UserName", item.UserName);
        //        result.Add("Role", item.Role);
        //        result.Add("Department", item.Department);
        //        //添加返回Token信息
    ...
    public class Users_ApiController : System.Web.Http.ApiController
    {
        private ApplicationDbContext db = new ApplicationDbContext();
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject CheckToken([System.Web.Http.FromBody]JObject data)
        {
            //空方法，检查Token可用性
            JObject result = new JObject();
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject UserInfo([System.Web.Http.FromBody]JObject data)
        {
            //从解密token中取出用户名(工号)
            AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
            string UserNumber = auth.UserNumber;

            JObject userInfo = new JObject();
            //int userNum = Convert.ToInt32(data["UserNumber"]);
            int userNum = Convert.ToInt32(UserNumber);
            var user = db.Users.Where(c => c.UserNum == userNum).FirstOrDefault();
            userInfo.Add("Id", user.ID);
            userInfo.Add("UserNumber", user.UserNum);
            userInfo.Add("Name", user.UserName);
            userInfo.Add("Role", user.Role);
            userInfo.Add("Department", user.Department);
            userInfo.Add("Admin", user.Role == "系统管理员" ? true : false);
            var limitsList = GetUserLimitsList(userNum);

            JObject result = new JObject();
            result.Add("PostResult", comm.ReturnApiPostStatus());
            result.Add("UserInfo", userInfo);
            result.Add("UserLimits", limitsList);
            return result;
        }

        private JObject GetUserLimitsList(int userNum)
        {
            JObject resultAll = new JObject();
            List<string> result_by_Array = new List<string>();
            //JObject result = new JObject();
            //根据工号拿到权限组
            var idlist = db.Useroles.Where(c => c.UserID == userNum).Select(c => new { c.FourModule, c.New_RolesName, c.New_Roles }).ToList();
            if (idlist == null) return new JObject();
            //取出模块清单（去重）
            var moduleList = idlist.Select(c => c.FourModule).Distinct().ToList();
            //根据权限组取出权限组对应的权限清单
            List<UserNewPermissions> userNewPermissionsList1 = new List<UserNewPermissions>();
            foreach (var it in idlist)
            {
                userNewPermissionsList1.AddRange(db.UserNewPermissions.Where(c => c.RolesName == it.New_RolesName).ToList());
            }
            JObject module_JOb = new JObject();
            //根据四大模块循环
            foreach (var it in moduleList)
            {
                //取出其中一个模块的用户组记录清单
                var moduleNewpermissionList = userNewPermissionsList1.Where(c => c.FourModule == it).ToList();
                var userLimitsNameList = idlist.Where(c => c.FourModule == it).ToList();
                JObject rolesName_JOb = new JObject();
                foreach (var limitsname in userLimitsNameList)
                {
                    //按其中一个权限组拆分用户的权限Code
                    if (!String.IsNullOrEmpty(limitsname.New_Roles))
                    {
                        string[] strCharArr = limitsname.New_Roles.Split(',');
                        int[] iNums;
                        iNums = Array.ConvertAll(strCharArr, int.Parse);
                        List<UserNewPermissions> roleslist = moduleNewpermissionList.Where(c => c.RolesName == limitsname.New_RolesName && iNums.Contains(c.RolesCode)).ToList();
                        JArray roleslist_JA = new JArray();
                        foreach (var ii in roleslist)
                        {
                            JObject temp = new JObject();
                            temp.Add(ii.RolesCode.ToString(), ii.Discription);
                            roleslist_JA.Add(temp);
                            result_by_Array.Add(ii.Discription);
                        }
                        rolesName_JOb.Add(limitsname.New_RolesName, roleslist_JA);

                        result_by_Array.Add(limitsname.New_RolesName);
                    }
                }
                if (rolesName_JOb.Count > 0) module_JOb.Add(it, rolesName_JOb);
                result_by_Array.Add(it);
            }
            resultAll.Add("result_by_module", module_JOb);
            resultAll.Add("result_by_Array", new JArray(result_by_Array.Distinct()));
            return resultAll;
        }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject Permiss_FourModuleList()
        {
            JArray result = new JArray();
            var Module = db.UserNewPermissions.OrderByDescending(m => m.id).Select(c => c.FourModule).Distinct().ToList();
            result.Add(Module);
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject Permiss_RolesName([System.Web.Http.FromBody]JObject data)
        {
            JArray result = new JArray();
            JObject table = new JObject();
            ...
            var ModuleList = db.UserNewPermissions.Where(c => c.FourModule == FourModule).Select(c => c.RolesName).Distinct().ToList();
            if (ModuleList == null)//如果找不到信息,则返回null
            {
                return common.GetModuleFromJarray(result);
            }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject Permiss_Discription([System.Web.Http.FromBody]JObject data)
        {
            JArray result = new JArray();
            JObject table = new JObject();
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            string RolesName = obj.RolesName == null ? null : obj.RolesName;//模块
            var ModuleList = db.UserNewPermissions.Where(c => c.RolesName == RolesName).Select(c => c.Discription).Distinct().ToList();
            if (ModuleList == null)//如果找不到信息,则返回null
            {
                return common.GetModuleFromJarray(result);
            }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject PersonnelList([System.Web.Http.FromBody]JObject data)
        {
            JArray result = new JArray();
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            ...
            List<UserNewPermissions> usersList = new List<UserNewPermissions>();
            if (!String.IsNullOrEmpty(FourModule))
            {
                usersList = db.UserNewPermissions.Where(c => c.FourModule == FourModule).ToList();
            }
            if (!String.IsNullOrEmpty(RolesName))
            {
                usersList = db.UserNewPermissions.Where(c => c.RolesName == RolesName).ToList();
            }
            if (!String.IsNullOrEmpty(Discription))
            {
                usersList = db.UserNewPermissions.Where(c => c.Discription == Discription).ToList();
            }
            if (usersList.Count > 0)
            {
                JArray moduleList = new JArray();
                var RolesNameList = usersList.Select(c => c.FourModule).Distinct().ToList();
                foreach (var four in RolesNameList)
                {
                    var discriptionList = usersList.Where(c => c.FourModule == four).Select(c => c.RolesName).Distinct().ToList();
                    foreach (var roles in discriptionList)
                    {

                        var codeList = usersList.Where(c => c.FourModule == four && c.RolesName == roles).Select(c => c.Discription).Distinct().ToList();
                        foreach (var discriList in codeList)
                        {
                            var distin = usersList.Where(c => c.FourModule == four && c.RolesName == roles && c.Discription == discriList).Select(c => c.RolesCode).Distinct().FirstOrDefault();
                            var nameList = db.Useroles.Where(c => c.FourModule == four && c.New_RolesName == roles).Select(c => new { c.New_Roles, c.UserID, c.UserName, c.Department }).ToList();
                            foreach (var item in nameList)
                            {
                                string[] strCharArr = item.New_Roles.Split(',');
                                int[] iNums;
                                iNums = Array.ConvertAll(strCharArr, int.Parse);
                                if (iNums.Contains(distin))
                                {
                                    JObject code = new JObject();
                                    code.Add("FourModule", four);
                                    code.Add("RolesName", roles);
                                    code.Add("Discription", discriList);
                                    code.Add("Department", item.Department == null ? null : item.Department);
                                    code.Add("UserID", item.UserID == 0 ? 0 : item.UserID);
                                    code.Add("UserName", item.UserName == null ? null : item.UserName);
                                    result.Add(code);
                                    code = new JObject();
                                }
                            }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject RolesNameList([System.Web.Http.FromBody]JObject data)
        {
            JArray result = new JArray();
            string FourModule = data["FourModule"] == null ? null : data["FourModule"].ToString();//模块
            var rolesName = db.UserNewPermissions.Where(c => c.FourModule == FourModule).Select(c => c.RolesName).Distinct().ToList();
            result.Add(rolesName);
            return common.GetModuleFromJarray(result);
        }

        //2.根据模块、权限组和根据模块、权限组、部门/姓名显示数据
        [HttpPost]
        [ApiAuthorize]
        public JObject DiscriptionList([System.Web.Http.FromBody]JObject data)
        {
            JArray result = new JArray();
            JObject table = new JObject();
            ...
            string RolesName = obj.RolesName == null ? null : obj.RolesName;
            string FourModule = obj.FourModule == null ? null : obj.FourModule;//模块         
            string Department = obj.Department == null ? null : obj.Department;//权限名             
            ...
            foreach (var item in users)
            {
                ...
                {
                    foreach (var it in discription)
                    {
                        rolesList.Add("RolesCode", it.RolesCode);
                        rolesList.Add("Discription", it.Discription);
                        rolesList.Add("Discr", false);
                        tableList.Add(rolesList);
                        rolesList = new JObject();
                    }
        ...
        //3.授权权限，取消权限（有调用第四点的方法）
        [HttpPost]
        [ApiAuthorize]
        public JObject Cancel_Authorization([System.Web.Http.FromBody]JObject data)
        {
            JObject result = new JObject();
            if (data == null)
            {
            ...
            string New_RolesName = obj.New_RolesName == null ? null : obj.New_RolesName;//权限组
            ...
            if (roleList.Count() == 0)
            {
                Deleterolename(UserID, New_RolesName);
                result.Add("Result", true);
                result.Add("Message", "操作成功");
                return common.GetModuleFromJobjet(result);
            }
            var checkUser = db.Useroles.Where(c => c.UserName == UserName && c.New_RolesName == New_RolesName && c.UserID == UserID);
            //如果没有这个用户信息，添加
            if (checkUser.Count() == 0)
            {
                Useroles userrole = new Useroles { UserName = UserName, UserID = UserID, Department = Department, Position = Position, New_Roles = rolecode, New_RolesName = New_RolesName, FourModule = FourModule };
                db.Useroles.Add(userrole);
                db.SaveChanges();
            }
            //如果有这个用户信息，修改roles
        ...
        //4.删除Useroles表里Roles字段为空的记录
        public void Deleterolename(int id, string rolesname)
        {
            var delete_roles = db.Useroles.Where(c => c.UserID == id && c.New_RolesName == rolesname).FirstOrDefault();
            var code = db.Useroles.Where(c => c.UserID == id && c.New_RolesName == rolesname).Select(c => c.New_Roles).ToList();
            foreach (var item in code)
            {
                if (item.Count() != 0)
                {
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject AddModule([System.Web.Http.FromBody]JObject data)
        {
            JObject result = new JObject();
            int count = 0;
            AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
            ...
            {
                result.Add("Result", false);
                result.Add("Message", "该模块已创建！");
                return common.GetModuleFromJobjet(result);
            }
            else
            {
                var module = new UserNewPermissions() { FourModule = FourModule, RolesName = "新权限组", RolesCode = 1, Discription = "查看功能", Operator = auth.UserName, OperateDT = DateTime.Now };
                db.UserNewPermissions.Add(module);
                count = db.SaveChanges();
                if (count > 0)
                {
                    result.Add("Result", true);
                    result.Add("Message", "添加成功！");
                    return common.GetModuleFromJobjet(result);
                }
                else
                {
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject ModifyModule([System.Web.Http.FromBody]JObject data)
        {
            JObject result = new JObject();
            int count = 0;
            AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
            ...
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            string fourModule = obj.fourModule == null ? null : obj.fourModule;//模块              
            string new_fourModule = obj.new_fourModule == null ? null : obj.new_fourModule;//修改后的模块
            string rolesName = obj.rolesName == null ? null : obj.rolesName;//权限名
            if (fourModule != null && new_fourModule != null && rolesName == null)
            {
                var original = db.UserNewPermissions.Where(c => c.FourModule == fourModule).ToList();
                foreach (var item in original)
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject AddPermission([System.Web.Http.FromBody]JObject data)
        {
            JObject result = new JObject();
            int count = 0;
            AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject ModifyRolesName([System.Web.Http.FromBody]JObject data)
        {
            JObject result = new JObject();
            int count = 0;
            AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            string new_rolesName = obj.new_rolesName == null ? null : obj.new_rolesName;//新权限名
            string rolesName = obj.rolesName == null ? null : obj.rolesName;//权限组
            var original = db.UserNewPermissions.Where(c => c.RolesName == rolesName).ToList();
            if (original != null)
            {
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject ModifyDiscription([System.Web.Http.FromBody]JObject data)
        {
            JObject result = new JObject();
            int count = 0;
            AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
            ...
            string discription = obj.discription == null ? null : obj.discription;//权限名
            string rolesName = obj.rolesName == null ? null : obj.rolesName;//权限组
            string new_discription = obj.new_discription == null ? null : obj.new_discription;//修改后的权限名
            var discrip = db.UserNewPermissions.Where(c => c.RolesName == rolesName && c.Discription == discription).ToList();
            if (discrip != null)
            {
                foreach (var it in discrip)
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject Batch_ADDusers([System.Web.Http.FromBody]JObject data)
        {
            JArray res = new JArray();
            JObject repat = new JObject();
            JObject result = new JObject();
            ...
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            List<Users> users = obj.users == null ? null : obj.users;
            if (users.Count() != 0)
            {
                foreach (var item in users)
                {
                    ...
                    item.UserNumber = item.UserNum.ToString();
                    if (db.Users.Count(c => c.UserNum == item.UserNum && c.UserName == item.UserName) > 0)
                    {
                        repat.Add("UserName", item.UserName);
                        repat.Add("UserNum", item.UserNum);
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject Single_ADDusers([System.Web.Http.FromBody]JObject data)
        {
            JArray res = new JArray();
            JObject repat = new JObject();
            JObject result = new JObject();
            ...
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            Users users = obj.users == null ? null : obj.users;
            if (users != null)
            {
                users.CreateDate = DateTime.Now;
                users.Creator = auth.UserName;
                if (db.Users.Count(c => c.UserNum == users.UserNum && c.UserName == users.UserName) > 0)
                {
                    repat.Add("UserName", users.UserName);
                    repat.Add("UserNum", users.UserNum);
                    res.Add(repat);
                    repat = new JObject();
                    if (res.Count > 0)
                    {
                ...
                users.UserNumber = users.UserNum.ToString();
                db.Users.Add(users);
                int savecount = db.SaveChanges();
                if (savecount > 0)
                {
        ...
        public JObject UserqueryList()
        {
            ...
            result.Add("Message", deaprList);
            deaprList = new JArray();
            result.Add("Message1", deaprList1);
            deaprList1 = new JArray();
            return common.GetModuleFromJobjet(result);
        }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject DeleteUser([System.Web.Http.FromBody]JObject data)
        {
            JObject result = new JObject();
            AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
            var jsonStr = JsonConvert.SerializeObject(data);
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject User_ManagementIndex([System.Web.Http.FromBody]JObject data)
        {
            JArray result = new JArray();
            JObject table = new JObject();
            AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            int? UserId = obj.UserId == 0 ? 0 : obj.UserId;//工号
            string UserName = obj.UserName == null ? null : obj.UserName;//用户名
            string Department = obj.Department == null ? null : obj.Department;//部门
            string Role = obj.Role == null ? null : obj.Role;//职位
            var UsersList = db.Users;
            List<Users> ListUsers = new List<Users>();
            if (UserId != null)
            {
                ListUsers = UsersList.Where(c => c.UserNum == UserId).ToList();
            }
            if (UserName != null)
            {
                ListUsers = UsersList.Where(c => c.UserName == UserName).ToList();
            }
            if (Department != null)
            {
                ListUsers = UsersList.Where(c => c.Department == Department).ToList();
            }
            if (Role != null)
            {
                ListUsers = UsersList.Where(c => c.Role == Role).ToList();
            }
            if (ListUsers.Count > 0)
            {
                var UserIdList = ListUsers.Select(c => c.UserNum).ToList();
                foreach (var item in UserIdList)
                {
                    var code = ListUsers.Where(c => c.UserNum == item).FirstOrDefault();
                    table.Add("Department", code.Department);//部门
                    table.Add("UserId", item);//工号
                    table.Add("UserName", code.UserName);//用户名
                    table.Add("Role", code.Role);//职位（角色）
                    table.Add("Description", code.Description);//描述
                    result.Add(table);
                    table = new JObject();
                }
            }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject Modify_Users([System.Web.Http.FromBody]JObject data)
        {
            JArray res = new JArray();
            JObject repat = new JObject();
            JObject result = new JObject();
