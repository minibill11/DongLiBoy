...
namespace JianHeMES.Controllers
{
    public class TestController : Controller
    {
        ...
        public JArray CheckModuleNeverInsideBoxList(string ordernum,string barcodetype="模块")
        {
            ...
            var list = mesdb.BarCodes.Where(c => c.OrderNum == ordernum && c.BarCodeType == barcodetype).Select(c => c.BarCodesNum).Except(mesdb.ModuleInsideTheBox.Where(d => d.OrderNum == ordernum).Select(c => c.ModuleBarcode)).OrderBy(c=>c).ToList();
            list.ForEach(c =>  result.Add(c.ToString()));
            return result;
        }

        public ActionResult CRM_Test()
        {
            //var result = CommonERPDB.ERP_ContractQuery("L231-20060005", "");
            var result = CommonERPDB.ERP_ContractQuery("", "H221-20050363");
            return View();
        }


        public ActionResult GanttTest()
        {


            return View();
        } 
        public ActionResult PrintBarcodeTest()
        {
        ...
        public ActionResult test111()
        {
            ...
            item.Add("name", "SMT");
            item.Add("mincapacity", 52.1);
            item.Add("moduleneed", 8);
            A.Add(item);
            item = new JObject();
            item.Add("name", "模块");
            item.Add("mincapacity", 85.4);
            item.Add("moduleneed", 5);
            A.Add(item);
            var e = A.Where(c => c["name"].ToString() == "SMT").FirstOrDefault();
            if (e == null)
            { }
            else
            {
                var aa = e["mincapacity"];
                e["mincapacity"] = 88.8;
            }
        ...
        {
            //开始绘制图片
            int initialWidth = 480, initialHeight = 240;//宽2高1
            ...
            SolidBrush bush = new SolidBrush(System.Drawing.Color.Black);//填充的颜色
            Pen pen = new Pen(Color.Black, 3);
            //画横线
            theGraphics.DrawLine(pen, 10, 72, 472, 72);
            System.Drawing.Font font;
            //标签
            Bitmap bmp_logo = new Bitmap(@"D:\\MES_Data\\LOGO_black.png");
            //double beishulogo = 0.95;
            theGraphics.DrawImage(bmp_logo, 20, 16, (float)(bmp_logo.Width), (float)(bmp_logo.Height));
            

            //文字 liantronice 电话.....
            font = new System.Drawing.Font("Microsoft YaHei UI", 15, FontStyle.Bold);//OCR-B//宋体
            theGraphics.DrawString("电话/Tel:+86-400-888-0158", font, bush, 224, 48);
            //型号
            font = new System.Drawing.Font("Microsoft YaHei UI", 15, FontStyle.Bold);//OCR-B//宋体
            theGraphics.DrawString("型号/Model:VPQ1.5", font, bush, 16, 80);
            //输入
            theGraphics.DrawString("输入/Input:AC220V.1.5A(Max.)50/60Hz", font, bush, 16, 104);
            //序列
            theGraphics.DrawString("序列/No. :2020-YA098-2. 13/30", font, bush, 16, 128);
            //日期
            theGraphics.DrawString("日期/Date:2020-05-07", font, bush, 16, 152);
            //3C图片
            Bitmap bmp_3c = new Bitmap(@"D:\\MES_Data\\3C.png");
            //double beishulogo = 0.95;
            theGraphics.DrawImage(bmp_3c, 24, 184, (float)(bmp_3c.Width), (float)(bmp_3c.Height));
            //中国制造
            font = new System.Drawing.Font("Microsoft YaHei UI", 15, FontStyle.Bold);//OCR-B//宋体
            theGraphics.DrawString("中国制造", font, bush, 344, 184);
            //madeinchine
            theGraphics.DrawString("Made In China", font, bush, 312, 208);
            //引入模组号
            StringFormat geshi = new StringFormat();
            geshi.Alignment = StringAlignment.Center; //居中
            int count = 0;

            string data = "^XA^JMA^MD" + concentration + "^LL245^PW485~DGR:ZONE.GRF,";//^MD5浓度
            Bitmap bm = new Bitmap(BarCodeLablePrint.ConvertTo1Bpp1(BarCodeLablePrint.ToGray(theBitmap)));//图形转二值
            bm = BarCodeLablePrint.RePic(bm, bm.Width, bm.Height);
            ...
            else
            {
                ...
                data += "^LH0,10^FO"+ right + ","+ down + "^XGR:ZONE.GRF^FS^XZ";
                IPPrinttest(data.ToString(), pagecount, ip, port);
        ...
        [HttpPost]
        public ActionResult b_a_list(string ordernum, decimal icsmt, decimal linesmt)
        {
            var icmultiple = 0;
            int linemultiple = 0;
            ...
            do
            {
                ...
                {
                    icmultiple++;
                    var tempic = icsmt * icmultiple;
                    linemultiple = (int)Math.Floor(tempic / linesmt);
                    var temp2 = linesmt * linemultiple;
                    if (temp2 > tempic)
                    ...
                    {
                        linemultiple = (int)Math.Ceiling(tempic / linesmt);
                        var temp3 = linesmt * linemultiple;
                        if (temp3 > tempic)
                ...
                {
                    linemultiple++;
                    var templine = linesmt * linemultiple;
                    icmultiple = (int)Math.Floor(templine / icsmt);
                    var temp2 = icsmt * icmultiple;
                    if (temp2 > templine)
                    ...
                    if (multipleRate < (decimal)0.85)
                    {
                        icmultiple = (int)Math.Ceiling(templine / icsmt);
                        var temp3 = icsmt * icmultiple;
                        if (temp3 > templine)
