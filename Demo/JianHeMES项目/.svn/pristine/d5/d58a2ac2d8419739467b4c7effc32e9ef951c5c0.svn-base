...
namespace JianHeMES.Controllers
{
    public class MetalPlateController : Controller
    {
        ...
        public ActionResult MetalPlate_Index()
        {
            if (Session["User"] == null)
            {
                return RedirectToAction("Login", "Users", new { col = "MetalPlate", act = "MetalPlate_Index" });
            }
            return View();
        }
        public ActionResult MetalPlate_CreatBasicInfo()
        {
            if (Session["User"] == null)
            {
                return RedirectToAction("Login", "Users", new { col = "MetalPlate", act = "MetalPlate_CreatBasicInfo" });
            }
            return View();
        }
        public ActionResult MetalPlate_CreatProduction()
        {
            if (Session["User"] == null)
            {
                return RedirectToAction("Login", "Users", new { col = "MetalPlate", act = "MetalPlate_CreatProduction" });
            }
            return View();
        }
        public ActionResult Modify()
        {
            if (Session["User"] == null)
            {
                return RedirectToAction("Login", "Users", new { col = "MetalPlate", act = "Modify" });
            }
            return View();
        }
        #endregion
            ...
            else
            {
                ...
                if (String.IsNullOrEmpty(str))
                {
                    foreach (var i in record)
                    {
                        ...
                        if (i.Section == "入库")
                        {
                            list.InputTime =i.InputTime;//等于入库采用入库传进的时间
                        }
                        else {
                            list.InputTime = DateTime.Now;//否则时候系统当前时间
                        }                       
                        db.MetalPlateProduction.Add(list);
                        savecout += db.SaveChanges();
                        if (i.Section == "入库")//修改基本信息表状态
                        {
        ...
        public ActionResult getProductionInfo(string[] ordernum)
        {
            ...
            if (orderList.Count > 0)
            {
                ...
                foreach (var item in orderList)
                {
                    ...
                    var DrawCompletion = Complete(item.Quantity, drawNormal + drawAbnormal);//完成率
                    var DrawPass = Complete(item.Quantity, drawNormal);//合格率
                    var DrawAbnormal = Complete(item.Quantity, drawAbnormal);//异常
                    basicinfo.Add("DrawCompletion", DrawCompletion == "0"?item.CompletionState==true?"/":"-%": DrawCompletion + "%" + "<br>" + "(" + (drawNormal + drawAbnormal).ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");
                    basicinfo.Add("DrawPass", DrawPass == "0"? item.CompletionState == true ? "/" : "-%" : DrawPass + "%" + "<br>" + "(" + drawNormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");
                    basicinfo.Add("DrawAbnormal", DrawAbnormal == "0" ? item.CompletionState == true ? "/" : "-%" : DrawAbnormal + "%" + "<br>" + "(" + drawAbnormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");
                    ...
                    var ProgramCompletion = Complete(item.Quantity, programmeNormal + programmeAbnormal);//完成率
                    var ProgramPass = Complete(item.Quantity, programmeNormal);//合格率
                    var ProgramAbnormal = Complete(item.Quantity, programmeAbnormal);//异常
                    basicinfo.Add("ProgramCompletion", ProgramCompletion == "0"? item.CompletionState == true ? "/" : "-%" : ProgramCompletion + "%" + "<br>" + "(" + (programmeNormal + programmeAbnormal).ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//完成率
                    basicinfo.Add("ProgramPass", ProgramPass == "0"? item.CompletionState == true ? "/" : "-%" : ProgramPass + "%" + "<br>" + "(" + programmeNormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//合格率
                    basicinfo.Add("ProgramAbnormal", ProgramAbnormal == "0" ? item.CompletionState == true ? "/" : "-%" : ProgramAbnormal + "%" + "<br>" + "(" + programmeAbnormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//异常
                    ...
                    var BluntCompletion = Complete(item.Quantity, bluntNormal + bluntAbnormal);//完成率
                    var BluntPass = Complete(item.Quantity, bluntNormal);//合格率
                    var BluntAbnormal = Complete(item.Quantity, bluntAbnormal);//异常
                    basicinfo.Add("BluntCompletion", BluntCompletion == "0" ? item.CompletionState == true ? "/": "-%" : BluntCompletion + "%" + "<br>" + "(" + (bluntNormal + bluntAbnormal).ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//完成率
                    basicinfo.Add("BluntPass", BluntPass == "0" ? item.CompletionState == true ? "/" :"-%" : BluntPass + "%" + "<br>" + "(" + bluntNormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//合格率
                    basicinfo.Add("BluntAbnormal", BluntAbnormal == "0" ? item.CompletionState == true ? "/": "-%" : BluntAbnormal + "%" + "<br>" + "(" + bluntAbnormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//异常
                    ...
                    var ChamferCompletion = Complete(item.Quantity, chamferNormal + chamferAbnormal);
                    var ChamferPass = Complete(item.Quantity, chamferNormal);
                    var ChamferAbnormal = Complete(item.Quantity, chamferAbnormal);
                    basicinfo.Add("ChamferCompletion", ChamferCompletion == "0" ? item.CompletionState == true ? "/" : "-%" : ChamferCompletion + "%" + "<br>" + "(" + (chamferNormal + chamferAbnormal).ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//完成率
                    basicinfo.Add("ChamferPass", ChamferPass == "0" ? item.CompletionState == true ? "/" : "-%" : ChamferPass + "%" + "<br>" + "(" + chamferNormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//合格率
                    basicinfo.Add("ChamferAbnormal", ChamferAbnormal == "0" ? item.CompletionState == true ? "/" : "-%" : ChamferAbnormal + "%" + "<br>" + "(" + chamferAbnormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//异常
                    ...
                    var BendCompletion = Complete(item.Quantity, bendNormal + bendAbnormal);//完成率
                    var BendPass = Complete(item.Quantity, bendNormal);//合格率
                    var BendAbnormal = Complete(item.Quantity, bendAbnormal);//异常
                    basicinfo.Add("BendCompletion", BendCompletion == "0" ? item.CompletionState == true ? "/" : "-%" : BendCompletion + "%" + "<br>" + "(" + (bendNormal + bendAbnormal).ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//完成率
                    basicinfo.Add("BendPass", BendPass == "0" ? item.CompletionState == true ? "/" : "-%" : BendPass + "%" + "<br>" + "(" + bendNormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//合格率
                    basicinfo.Add("BendAbnormal", BendAbnormal == "0" ? item.CompletionState == true ? "/" : "-%" : BendAbnormal + "%" + "<br>" + "(" + bendAbnormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//异常
                    ...
                    var WeldCompletion = Complete(item.Quantity, weldNormal + weldAbnormal);//完成率
                    var WeldPass = Complete(item.Quantity, weldNormal);//合格率
                    var WeldAbnormal = Complete(item.Quantity, weldAbnormal);//异常
                    basicinfo.Add("WeldCompletion", WeldCompletion == "0" ? item.CompletionState == true ? "/" : "-%" : WeldCompletion + "%" + "<br>" + "(" + (weldNormal + weldAbnormal).ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//完成率
                    basicinfo.Add("WeldPass", WeldPass== "0" ? item.CompletionState == true ? "/" : "-%" : WeldPass + "%" + "<br>" + "(" + weldNormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//合格率
                    basicinfo.Add("WeldAbnormal", WeldAbnormal== "0" ? item.CompletionState == true ? "/" : "-%" : WeldAbnormal + "%" + "<br>" + "(" + weldAbnormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//异常
                    ...
                    var SprayCompletion = Complete(item.Quantity, sprayNormal + sprayAbnormal);//完成率
                    var SprayPass = Complete(item.Quantity, sprayNormal);//合格率
                    var SprayAbnormal = Complete(item.Quantity, sprayAbnormal);//异常
                    basicinfo.Add("SprayCompletion", SprayCompletion == "0" ? item.CompletionState == true ? "/" : "-%" : SprayCompletion + "%" + "<br>" + "(" + (sprayNormal + sprayAbnormal).ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//完成率
                    basicinfo.Add("SprayPass", SprayPass == "0" ? item.CompletionState == true ? "/" : "-%" : SprayPass + "%" + "<br>" + "(" + sprayNormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//合格率
                    basicinfo.Add("SprayAbnormal", SprayAbnormal == "0" ? item.CompletionState == true ? "/" : "-%" : SprayAbnormal + "%" + "<br>" + "(" + sprayAbnormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//异常
                    ...
                    var AssembleCompletion = Complete(item.Quantity, assembleNormal + assembleAbnormal);//完成率
                    var AssemblePass = Complete(item.Quantity, assembleNormal);//合格率
                    var AssembleAbnormal = Complete(item.Quantity, assembleAbnormal);//异常
                    basicinfo.Add("AssembleCompletion", AssembleCompletion == "0"? item.CompletionState == true ? "/" : "-%" : AssembleCompletion + "%" + "<br>" + "(" + (assembleNormal + assembleAbnormal).ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//完成率
                    basicinfo.Add("AssemblePass", AssemblePass == "0" ? item.CompletionState == true ? "/" : "-%" : AssemblePass + "%" + "<br>" + "(" + assembleNormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//合格率
                    basicinfo.Add("AssembleAbnormal", AssembleAbnormal== "0" ? item.CompletionState == true ? "/" : "-%" : AssembleAbnormal + "%" + "<br>" + "(" + assembleAbnormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//异常
                    ...
                    basicinfo.Add("WarehouseCompletion", Complete(item.Quantity, warehouseNormal + warehouseAbNormal) == "0" ? "-%" : Complete(item.Quantity, warehouseNormal + warehouseAbNormal) + "%" + "<br>" + "(" + (warehouseNormal + warehouseAbNormal).ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//完成率
                    basicinfo.Add("WarehousePass", Complete(item.Quantity, warehouseNormal) == "0" ? "-%" : Complete(item.Quantity, warehouseNormal) + "%" + "<br>" + "(" + warehouseNormal.ToString("0.##") + "/" + item.Quantity.ToString("0.##") + ")");//合格率
    ...
    public class MetalPlateAPIController : System.Web.Http.ApiController
    {
        private ApplicationDbContext db = new ApplicationDbContext();
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject DelBasicInfo([System.Web.Http.FromBody]JObject data)//MetalPlate_BasicInfo record   方法名：delteBasicInfo
        {
            //1.根据id判断该记录的订单状态是否已完成  
            //2.已完成 不允许删除   
            //3.未完成：使用订单号，生产类型到数据表找记录 如果有此订单、类型对应的记录存在  提示用户先删除数据的数据 在删除基本信息表的数据
            ...
            string order = obj.OrderNum;
            string type = obj.ProductionType;
            int Id = obj.Id;
            var basicList = db.MetalPlate_BasicInfo.Where(c => c.OrderNum == order && c.ProductionType == type && c.CompletionState == true).ToList().Count();
            if (basicList > 0)
            {
                return common.GetModuleFromJobjet(result,false,"此订单已完成入库，不允许删除数据");
            }
            else
            {
                var productionList = db.MetalPlateProduction.Where(c => c.OrderNum == order && c.ProductionType == type).ToList().Count();
                if (productionList > 0)
                {
                    return common.GetModuleFromJobjet(result,false, "此订单的生产数据未删除,需先删除生产数据,在删除基本数据！");
                }
                else
                {
                    var removeList = db.MetalPlate_BasicInfo.Where(c => c.Id == Id).ToList();
                    db.MetalPlate_BasicInfo.RemoveRange(removeList);
                    int count = db.SaveChanges();
                    if (count > 0)
                    {
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject DelProduction([System.Web.Http.FromBody]JObject data)//MetalPlateProduction record   原方法名：delteProduction
        {
            JObject result = new JObject();
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            ...
            string type = obj.ProductionType;
            int Id = obj.Id;
            var list = db.MetalPlate_BasicInfo.Where(c => c.OrderNum == order && c.ProductionType == type && c.CompletionState == true).ToList().Count();
            if (list > 0)//大于零表示此订单已完成 不允许删除
            {
                return common.GetModuleFromJobjet(result,false, "此订单已完成入库，不允许删除数据");
            }
            ...
            {
                var removeList = db.MetalPlateProduction.Where(c => c.Id == Id).ToList();
                db.MetalPlateProduction.RemoveRange(removeList);
                int count = db.SaveChanges();
                if (count > 0) {
                    return common.GetModuleFromJobjet(result,true, "删除成功！");                   
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject MetalPlateProduction_modify([System.Web.Http.FromBody]JObject data)//MetalPlateProduction newList
        {
            JObject result = new JObject();
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            MetalPlateProduction newList = obj;
            //使用订单号，工段、生产类型找出已有数量
            var checkNumNo = db.MetalPlateProduction.Where(c => c.OrderNum == newList.OrderNum && c.Section == newList.Section && c.ProductionType == newList.ProductionType && c.Id != newList.Id).Select(c => c.NormalQuantity).ToArray().Sum();
            var checkNumabNo = db.MetalPlateProduction.Where(c => c.OrderNum == newList.OrderNum && c.Section == newList.Section && c.ProductionType == newList.ProductionType && c.Id != newList.Id).Select(c => c.AbnormaQuantity).ToArray().Sum();
            var record = db.MetalPlate_BasicInfo.Where(c => c.OrderNum == newList.OrderNum && c.ProductionType == newList.ProductionType).FirstOrDefault();
            ...
            if (total > record.Quantity)
            {
                return common.GetModuleFromJobjet(result,false,"修改失败,录入数量大于订单总数！");
            }
            else
            {
                DbEntityEntry<MetalPlateProduction> entry = db.Entry(newList);
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject MetalPlateBasicInfo_modify([System.Web.Http.FromBody]JObject data)//MetalPlate_BasicInfo newList
        {
            JObject result = new JObject();
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            MetalPlate_BasicInfo newList = obj;
            DbEntityEntry<MetalPlate_BasicInfo> entry = db.Entry(newList);
            entry.State = System.Data.Entity.EntityState.Modified;
            int count = db.SaveChanges();
            if (count > 0)
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject GainProduction([System.Web.Http.FromBody]JObject data)//string ordernum   原方法名：getProduction
        {
            JObject result = new JObject();
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            ...
            result.Add("Data", JsonConvert.SerializeObject(orderList));
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject GainBasicInfo([System.Web.Http.FromBody]JObject data)//原方法参数：string ordernum  //原方法名：getBasicInfo
        {
            JObject result = new JObject();
            List<MetalPlate_BasicInfo> record = new List<MetalPlate_BasicInfo>();
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            string ordernum = obj.ordernum;
            var resultList = db.MetalPlate_BasicInfo.Where(k => k.OrderNum == ordernum).ToList();
            ...
            result.Add("Data", JsonConvert.SerializeObject(record));
            return common.GetModuleFromJobjet(result, true, "查询成功！");
        }
        #endregion
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject CheckBasicInfo([System.Web.Http.FromBody]JObject data)//string ordernum, string productionType
        {
            JObject result = new JObject();
            var jsonStr = JsonConvert.SerializeObject(data);
            var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
            string ordernum = obj.ordernum;
            string productionType = obj.productionType;
            var list = db.MetalPlate_BasicInfo.Where(k => k.OrderNum == ordernum && k.ProductionType == productionType).ToList().Count();
            if (list > 0) return common.GetModuleFromJobjet(result,true,"已创建订单基本信息！");
            else return common.GetModuleFromJobjet(result,false,"此订单未创建基本信息记录，用户需先创建基本信息记录！");
        }
        #endregion
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject GainEnteredDatat([System.Web.Http.FromBody]JObject data)//原方法名：GetEnteredDatat     原方法参数：string ordernum, string productionType, string section
        {
            JObject resule = new JObject();
            var obj = JsonConvert.DeserializeObject<dynamic>(JsonConvert.SerializeObject(data));
            string ordernum = obj.ordernum;
            string productionType = obj.productionType;
            string section = obj.section;
            var list = db.MetalPlateProduction.Where(c => c.OrderNum == ordernum && c.ProductionType == productionType && c.Section == section).Select(c => new { c.OrderNum, c.ProductionType, c.Section, c.NormalQuantity, c.AbnormaQuantity }).ToList();
            resule.Add("Data",JsonConvert.SerializeObject(list));
            return common.GetModuleFromJobjet(resule,true,"查询成功！");
        }
        #endregion
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject GainOrderList()//原方法名：GetOrderList
        {
            var orders = db.MetalPlate_BasicInfo.Where(m => m.CompletionState == true).Select(m => m.OrderNum).Distinct().ToList(); 
            JArray result = new JArray();
            foreach (var item in orders)
            {
                JObject List = new JObject();
        ...
        [HttpPost]
        public JObject GainOrderNumList()//原方法名：GetOrderNumList
        {
            var orders = db.MetalPlate_BasicInfo.Select(m => m.OrderNum).Distinct().ToList();    //增加.Distinct()后会重新按OrderNum升序排序
            JArray result = new JArray();
            foreach (var item in orders)
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject getProductionInfo([System.Web.Http.FromBody]JObject data)//原方法名：getProductionInfo    原方法参数:string[] ordernum
        {
            JArray array = (JArray)JsonConvert.DeserializeObject(data["ordernum"].ToString());
            string[] ordernum = array.ToObject<List<string>>().ToArray();
            List<MetalPlate_BasicInfo> orderList = new List<MetalPlate_BasicInfo>();
            if (ordernum != null)
            {
                foreach (var item in ordernum)
            ...
            if (orderList.Count > 0)
            {
                ...
                //先找出订单号
                return common.GetModuleFromJarray(resultList,true,"查询成功！");
            }
            else return common.GetModuleFromJarray(null,false,"找不到已完成订单对应记录！");
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject CreatProductionInfo([System.Web.Http.FromBody]JObject data)//原方法参数：List<MetalPlateProduction> record, string department, string group
        {
            var obj = JsonConvert.DeserializeObject<dynamic>(JsonConvert.SerializeObject(data));
            ...
            List<MetalPlateProduction> record = (List<MetalPlateProduction>)JsonHelper.jsonDes<List<MetalPlateProduction>>(data["record"].ToString());
            string order = "";
            foreach (var j in record)
            {
                var basicInfoQty = db.MetalPlate_BasicInfo.Where(c => c.OrderNum == j.OrderNum && c.ProductionType == j.ProductionType).ToList();
            ...
            //如果有订单未录入基本信息，那么传进来的所有数据都不进行保存的动作
            if (!String.IsNullOrEmpty(order))
                return common.GetModuleFromJobjet(null,false,"保存失败，订单号为：" + order + "未录入基本信息!");
            else
            {
                string str = "";
                foreach (var item in record)
                ...
                if (String.IsNullOrEmpty(str))
                {
                    foreach (var i in record)
                    {
                        ...
                        list.Department = department;
                        list.Group = group;
                        list.InputPerson = auth.UserName;
                        if (i.Section == "入库")
                        {
                            list.InputTime = i.InputTime;//等于入库采用入库传进的时间
                        }
                ...
                else return common.GetModuleFromJobjet(null,false,"订单号为：" + str + "，录入的数量大于订单总数，记录保存失败！");
                if (savecout == record.Count()) return common.GetModuleFromJobjet(null,true,"保存成功！");
                else return common.GetModuleFromJobjet(null,false,"保存失败！");
            }
        }
        #endregion
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject CreatBasicInfo(List<MetalPlate_BasicInfo> record)
        {
            int savecout = 0;
            string str = "";
            foreach (var item in record)
            ...
            if (savecout == record.Count()) return common.GetModuleFromJobjet(null,true,"保存成功！");
            else return common.GetModuleFromJobjet(null,false,"生产类型为：" + str + "的记录已存在，保存失败！");
        }
        #endregion
    }
}
