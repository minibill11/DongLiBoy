...
namespace JianHeMES.Controllers
{
    public class FinalQCsController : Controller
    {
        ...
        public async Task<ActionResult> Index(string orderNum, string repetition, string FQCNormal, string Remark, int PageIndex = 0)
        {
            ...
            List<FinalQC> BarCodesList = (from m in db.FinalQC where m.OrderNum == orderNum select m).ToList();
            ...
            {
                if ((from m in db.FinalQC where m.BarCodesNum == barcode.BarCodesNum select m).Count() == 0)
                {
            ...
            foreach (var barcode in BarCodesList)
            {
                if (db.FinalQC.Where(m => m.BarCodesNum == barcode.BarCodesNum).Where(m => m.FQCCheckBT != null && m.FQCCheckFT == null).Count() > 0)
                {
        ...
        public async Task<ActionResult> FinalQC_B([Bind(Include = "Id,OrderNum,BarCodesNum,FQCCheckBT,FQCPrincipal,FQCCheckFT,FQCCheckDate,FQCCheckTime,FQCCheckTimeSpan,FinalQC_FQCCheckAbnormal,RepetitionFQCCheck,RepetitionFQCCheckCause,FQCCheckFinish,Remark")] FinalQC finalQC, string nuoBarCode=null)
        {
            ViewBag.OrderList = GetOrderList();//向View传递OrderNum订单号列表.
            var recount = 0;
            ...
            //创建一个集合存放此条码对应的记录
            var fqc_recordlist = db.FinalQC.Where(c => c.BarCodesNum == finalQC.BarCodesNum);
                    ...
                    if (finalQC.RepetitionFQCCheck == false)  //重复FQC要求不能勾上
                    {
                        ...
                        if (ModelState.IsValid)
                        {
                            ...
                            if (!string.IsNullOrEmpty(nuoBarCode)&&recount==0)
                                comm.InsertRelation(nuoBarCode, finalQC.BarCodesNum, finalQC.OrderNum, "FQC");
