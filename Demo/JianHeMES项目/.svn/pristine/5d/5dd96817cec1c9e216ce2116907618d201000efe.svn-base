...
namespace JianHeMES.Controllers
{
    public class Burn_inController : Controller
    {
        ...
        public ActionResult BurnInShelfQuery(string OrderNum, string BurnInShelfNum)
        {
            ...
            List<Burn_in> AllBurn_inRecords = db.Burn_in.Where(c => c.OQCCheckFT == null && c.OQCCheckBT != null && c.OQCCheckFinish == false).ToList();
            if (!string.IsNullOrEmpty(OrderNum))
            {
        ...
        public ActionResult BurnInShelfQueryHistory(string OrderNum, string BurnInShelfNum)
        {
            ...
            List<Burn_in> AllBurn_inRecords = db.Burn_in.Where(c => c.OQCCheckFT != null && c.OQCCheckBT != null && c.OQCCheckFinish == true).ToList();
            if (!string.IsNullOrEmpty(OrderNum))
            {
                AllBurn_inRecords = AllBurn_inRecords.Where(c => c.OrderNum == OrderNum).ToList();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Burn_in_B([Bind(Include = "Id,OrderNum,BurnInShelfNum,BarCodesNum,OQCCheckBT,OQCPrincipal,OQCCheckFT,OQCCheckTime,OQCCheckTimeSpan,Burn_in_OQCCheckAbnormal,RepairCondition,OQCCheckFinish")] Burn_in burn_in,string nuoBarCode=null)
        {
            ViewBag.OrderList = GetOrderList();//向View传递OrderNum订单号列表.
            var recount = 0;
            if (Session["User"] == null)
            ...
            if (db.Burn_in.Count(u => u.BarCodesNum == burn_in.BarCodesNum) == 0)//没有记录
            {
                if (burn_in.OrderNum == db.BarCodes.Where(u => u.BarCodesNum == burn_in.BarCodesNum).FirstOrDefault().OrderNum)
                {
                    ...
                    if (!string.IsNullOrEmpty(nuoBarCode) && recount == 0)
                        comm.InsertRelation(nuoBarCode, burn_in.BarCodesNum, burn_in.OrderNum, "OQC");
        ...
        #region ---------------批量模组开始老化功能------------
        [HttpPost]
        public ActionResult Burn_in_Batch_Begin(string OrderNum, string BurnInShelfNum, List<string> BarCodesNumList)
        {
