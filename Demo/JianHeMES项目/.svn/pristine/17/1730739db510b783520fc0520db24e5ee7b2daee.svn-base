...
namespace JianHeMES.Controllers
{
    public class AppearancesController : Controller
    {
        ...
        public ActionResult New_Index()
        {
            ...
            {
                return RedirectToAction("Login2", "Users", new { col = "Appearances", act = "New_Index" });
            }
            return View();
        }
        public ActionResult New_Appearance()
        {
            if (Session["User"] == null)
            {
                return RedirectToAction("Login2", "Users", new { col = "Appearances", act = "New_Appearance" });
            }
            return View();
        }
        #region --------------------OQCNormal列表
        ...
        public async Task<ActionResult> Appearance_F([Bind(Include = "Id,OrderNum,ToOrderNum,BarCodesNum,CustomerBarCodesNum,ModuleGroupNum,OQCCheckBT,OQCPrincipal,OQCCheckFT,OQCCheckDate,OQCCheckTime,OQCCheckTimeSpan,Appearance_OQCCheckAbnormal,RepairCondition,OQCCheckFinish,Remark,Department1,Group")] Appearance appearance)
        {
            ...
            if (ModelState.IsValid)
            {
                var modify_ModuleGroupNumber = await ModifyModuleFromAppearance_F(appearance.BarCodesNum, appearance.ModuleGroupNum, ((Users)Session["User"]).UserName);
                db.Entry(appearance).State = EntityState.Modified;
                await db.SaveChangesAsync();
                common.DelteMudole(appearance.OrderNum, appearance.ModuleGroupNum);
        ...
        public async Task<ActionResult> ModifyModuleFromAppearance_F(string barcodes, string mudule, string name)
        {
            var barcode = db.BarCodes.Where(C => C.BarCodesNum == barcodes).FirstOrDefault();
            barcode.ModuleGroupNum = mudule;
            UserOperateLog log = new UserOperateLog() { OperateDT = DateTime.Now, Operator = name, OperateRecord = "外观电检完成时,条码" + barcodes + "修改后的模组号是" + mudule };
        ...
        #region --------------------新的订单号列表------
        private ActionResult OrderNumList()
        {
        ...
        public ActionResult InsideBoxLablePrint(int pagecount, string barcode = "", string modulenum = "", string ip = "", int port = 0, int concentration = 5, bool testswitch = false)
        {
            ...
            if (testswitch == true)
            {
                ...
                theBitmap.Dispose();
                var aa = File(ms.ToArray(), "image/Png");
        ...
        public ActionResult UpdateMuduleFromCompleteAppearances(string modulenum, string barcode)
        {
            ...
            {
                JObject jobject = common.UpdateTotalModule(barcode, modulenum, ((Users)Session["User"]).UserName);
                ...
                {
                    result.Add("message", jobject["mes"].ToString());
                    result.Add("result", false);
                    return Content(JsonConvert.SerializeObject(result));
                }
            }
        ...
        public ActionResult DisplayTotalModule(string ordernum, string type = "模组")
        {
            ...
            var tempcalibtation = db.CalibrationRecord.Where(c => (c.OldBarCodesNum == null || c.OldBarCodesNum == c.BarCodesNum) && c.OrderNum == ordernum).Select(c => new TempCalibration() { barcode = c.BarCodesNum, module = c.ModuleGroupNum, id = c.ID, oldbarcode = c.OldBarCodesNum }).ToList();//校正信息
            var tempappearance = db.Appearance.Where(c => (c.OldBarCodesNum == null || c.OldBarCodesNum == c.BarCodesNum) && c.OrderNum == ordernum).Select(c => new TempAppearabce() { barcode = c.BarCodesNum, module = c.ModuleGroupNum, id = c.Id, oldbarcode = c.OldBarCodesNum }).ToList();//电检信息
            if (System.IO.File.Exists(@"D:\MES_Data\TemDate\OrderSequence2\" + ordernum + ".json") == true)
            {
                ...
                if (json.Count(c => c.ToString() == "已打印") != 0)
                {
                    rule = true;
                }
                else
            ...
            foreach (var item in barcoldeList)
            {
                ...
                var calibtationmodule = tempcalibtation.Where(c => c.barcode == item.BarCodesNum && (c.oldbarcode == null || c.oldbarcode == item.BarCodesNum)).Select(c => c.module).FirstOrDefault();
                value.Add("calibtationModule", calibtationmodule);//校正模组
                var appearanmodule = tempappearance.Where(c => c.barcode == item.BarCodesNum && (c.oldbarcode == null || c.oldbarcode == item.BarCodesNum)).Select(c => c.module).FirstOrDefault();
                ...
                if (rule)
                {
                    value.Add("ModuleNumPrint", "未绑定条码编号已打印");
                }
                else if (item.ModuleNumPrintCount == 0)
                {
    ...
    public class Appearances_ApiController : System.Web.Http.ApiController
    {
        private ApplicationDbContext db = new ApplicationDbContext();
        private CommonalityController comm = new CommonalityController();
        private CommonController common = new CommonController();
        ...
        public JObject NewIndex([System.Web.Http.FromBody]JObject data)
        {
            ...
            var info = db.Appearance.Where(c => c.OrderNum == ordernum && (c.OldOrderNum == null || c.OldOrderNum == ordernum)).Select(c => new CommonController.TempIndex
            {
                ...
                Abnormal = c.Appearance_OQCCheckAbnormal,
                id = c.Id
            }).ToList();
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject Appearance_B([System.Web.Http.FromBody]JObject data)
        {
            string BarCodesNum = data["BarCodesNum"].ToString();
            string OrderNum = data["OrderNum"].ToString();
            string Department = data["Department"].ToString();
            string Group = data["Group"].ToString();
            string CustomerBarCodesNum = data["CustomerBarCodesNum"].ToString();//客户条码

            JObject result = new JObject();
            var burn_in_Finish_Record = db.Burn_in.Where(c => c.BarCodesNum == BarCodesNum && c.OQCCheckFinish == true && (c.OldBarCodesNum == null || c.OldBarCodesNum == BarCodesNum)).Count();
            //检查条码和订单是否准确
            ...
            //检查条码是否完成老化
            else if (burn_in_Finish_Record == 0)
            {
                result.Add("id", null);
                result.Add("moduleNum", null);
                result.Add("startTime", null);
                return common.GetModuleFromJobjet(result,false, "此模组老化调试工序未完成！");
            }
            else
            {
                var startStatu = comapi.CheckSectionStart(BarCodesNum, "Appearances", "BarCodesNum", "OQCCheckFT", "OQCCheckFinish");
                if (startStatu == "新增")
                {
                    Appearance appearance = new Appearance();
                    appearance.OrderNum = OrderNum;
                    appearance.BarCodesNum = BarCodesNum;
                    appearance.OldOrderNum = OrderNum;
                    appearance.OldBarCodesNum = BarCodesNum;
                    appearance.OQCCheckBT = DateTime.Now;
                    appearance.Department1 = Department;
                    appearance.Group = Group;
                    appearance.CustomerBarCodesNum = CustomerBarCodesNum;
                    appearance.ModuleGroupNum = (from m in db.BarCodes where m.BarCodesNum == BarCodesNum select m).FirstOrDefault().ModuleGroupNum;
                    db.Appearance.Add(appearance);
                    db.SaveChangesAsync();
                    result.Add("id", appearance.Id);
                    result.Add("moduleNum", appearance.ModuleGroupNum);
                    result.Add("startTime", appearance.OQCCheckBT);
                    return common.GetModuleFromJobjet(result,true, "成功");
                }
                else if (startStatu == "修改")
                {
                    var appearance_doing_record = db.Appearance.FirstOrDefault(u => u.OQCCheckBT != null && u.OQCCheckFT == null && u.BarCodesNum == BarCodesNum && (u.OldBarCodesNum == BarCodesNum || u.OldBarCodesNum == null));//读出第一个正在做外观包装的记录
                    
                    result.Add("id", appearance_doing_record.Id);
                    result.Add("moduleNum", appearance_doing_record.ModuleGroupNum);
                    result.Add("startTime", appearance_doing_record.OQCCheckBT);
                    return common.GetModuleFromJobjet(result,true, "成功");
                }
                else if (startStatu == "重复")
                {
                    ...
                    result.Add("moduleNum", null);
                    result.Add("startTime", null);
                    return common.GetModuleFromJobjet(result,false, "此模组已经完成外观包装，不能对已通过外观包装的模组进行重复外观包装！");
                }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject Appearance_F([System.Web.Http.FromBody]JObject data)
        {
            /*
             * Appearance appearance, string UserName, string nuoOrder = null, string nuoBarCode = null, string isnuo = null
             */
            int id = int.Parse(data["id"].ToString());
            string UserName = data["UserName"].ToString();
            string RepairCondition = data["RepairCondition"].ToString();//维修情况
            string Appearance_OQCCheckAbnormal = data["Appearance_OQCCheckAbnormal"].ToString();//异常信息
            string ModuleGroupNum = data["ModuleGroupNum"].ToString();//模组号
            string Remark = data["Remark"].ToString();//维修情况
            string nuoOrder = data["nuoOrder"].ToString();//维修情况
            string nuoBarCode = data["nuoBarCode"].ToString();//维修情况
            string isnuo = data["isnuo"].ToString();//维修情况

            JObject result = new JObject();
            var appearance = db.Appearance.Find(id);
            if (appearance.OQCCheckFT == null)
            {
                //时间计算
                appearance.OQCCheckFT = DateTime.Now;
                JObject time = comapi.CalculateTimespan(appearance.OQCCheckBT.Value, appearance.OQCCheckFT.Value);
                appearance.OQCCheckTime = TimeSpan.Parse(time["Time"].ToString());
                appearance.OQCCheckDate = int.Parse(time["Date"].ToString());
                appearance.OQCCheckTimeSpan = time["TimeSpan"].ToString();

                appearance.OQCPrincipal = UserName;
                appearance.Appearance_OQCCheckAbnormal = Appearance_OQCCheckAbnormal;
                appearance.RepairCondition = RepairCondition;
                appearance.Remark = Remark;
                if (Appearance_OQCCheckAbnormal == "正常" && appearance.RepairCondition == "正常")
                {
                    appearance.OQCCheckFinish = true;
                }
                else
                {
                    appearance.OQCCheckFinish = false;
                }
            }
            //判断输入的模组号是否重复
            JObject jobject = comapi.UpdateTotalModule(appearance.OrderNum, appearance.BarCodesNum, ModuleGroupNum, UserName);

            //如果修改成功
            if (jobject["pass"].ToString() == "True")
            {
                appearance.ModuleGroupNum = ModuleGroupNum;
                db.Entry(appearance).State = EntityState.Modified;
                db.SaveChangesAsync();
                UserOperateLog log = new UserOperateLog() { OperateDT = DateTime.Now, Operator = UserName, OperateRecord = "外观电检完成时,条码" + appearance.BarCodesNum + "修改后的模组号是" + ModuleGroupNum };
                db.UserOperateLog.Add(log);
                db.SaveChangesAsync();
                //如果有json文件删除模组号
                common.DelteMudole(appearance.OrderNum, ModuleGroupNum);
                //如果有挪用
                if (!string.IsNullOrEmpty(nuoBarCode) && isnuo == "true")
                {
                    BarCodeRelation barcoderelation = new BarCodeRelation() { OldOrderNum = nuoOrder, OldBarCodeNum = nuoBarCode, NewBarCodesNum = appearance.BarCodesNum, NewOrderNum = appearance.OrderNum, Procedure = "Appearances", UsserID = 0, CreateDate = DateTime.Now };
                    if (!comm.InsertRelation(barcoderelation))
                    {
            ...
            return common.GetModuleFromJobjet(result,false,"失败");
        }
        
        #region    --------------------查询订单已完成、未完成、未开始条码
        [HttpPost]
        [ApiAuthorize]
        public JObject AppearanceChecklist([System.Web.Http.FromBody]JObject data)
        {
            string orderNum = data["orderNum"].ToString();
            ...
            {
                //调出订单对应全部记录      
                var AllAppearance = db.Appearance.Where(c => c.OrderNum == orderNum && (c.OldOrderNum == null || c.OldOrderNum == orderNum)).Select(c => new Common_ApiController.Temp { id = c.Id, BarCodesNum = c.BarCodesNum, Finsh = c.OQCCheckFinish, FinshTime = c.OQCCheckFT }).ToList();
                stationResult = comapi.SectionCehckList(orderNum, AllAppearance);
            }
        ...
        public JObject DisplayBarcode([System.Web.Http.FromBody]JObject data)
        {
            ...
            if (calBarcode != null && barcode != null)//如果两个模组号都不为空
            {
                if (calBarcode == barscode)
                {
                    return common.GetModuleFromJobjet(result,true, "此条码校正模组号为:" + calBarcode + "，条码表模组号为:" + barscode);
                }
                else
                {
                    return common.GetModuleFromJobjet(result,false, "此条码校正模组号为:" + calBarcode + "，条码表模组号为:" + barscode);
                }

            }
            else if (calBarcode == null && barscode == null)//如果两个都为空
            {
                return common.GetModuleFromJobjet(result,true, "此条码没有模组号");
            }
            else
            {
                return common.GetModuleFromJobjet(result,true, (string.IsNullOrEmpty(calBarcode) ? "YES, 此模组条码未完成校正" : "YES,此条码校正模组号为:" + calBarcode) + "，条码表模组号为:" + barscode);
            }
        }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject InsideBoxLablePrint([System.Web.Http.FromBody]JObject data)
        {
            /*
             * int pagecount, string barcode = "", string modulenum = "", string ip = "", int port = 0, int concentration = 5, bool testswitch = false
             */
            // JObject result = new JObject();
            int pagecount = int.Parse(data["pagecount"].ToString());
            int port = int.Parse(data["port"].ToString());
            int concentration = int.Parse(data["concentration"].ToString());
            bool testswitch = bool.Parse(data["testswitch"].ToString());
            string barcode = data["barcode"].ToString();
            string modulenum = data["modulenum"].ToString();
            ...
            else
            {
                ...
                data1 += "^LH0,0^FO150,0^XGR:ZONE.GRF^FS^XZ";
                ZebraUnity.IPPrint(data1.ToString(), pagecount, ip, port);
                result.Add("mes", "");
                return common.GetModuleFromJobjet(result);
            }
        }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject InsideBoxLablePrint2([System.Web.Http.FromBody]JObject data)
        {
            /*
             * string bitmap = "", int pagecount = 1, string barcode = "", string modulenum = "", int concentration = 5
             */
            int pagecount = int.Parse(data["pagecount"].ToString());
            int concentration = int.Parse(data["concentration"].ToString());
            string bitmap = data["bitmap"].ToString();
            string barcode = data["barcode"].ToString();
            string modulenum = data["modulenum"].ToString();
            ...
            if (!String.IsNullOrEmpty(barcode) && !String.IsNullOrEmpty(modulenum))
            {
                ...
                string data_e = "^XZ";
                bool result = BarCodeLablePrint.SendStringToPrinter(ptrn, data_s + modulenum_data + barcode_data + data_e);
                total.Add("mes", result ? "打印成功" : "打印失败");
                total.Add("pass", result);
                return common.GetModuleFromJobjet(total);
            }
            else
            {
                Bitmap theBitmap = ZebraUnity.Base64ToBitmap(bitmap);//把base64字符串转换成Bitmap
                ...
                if (pagecount == 1)
                {
                    if (BarCodeLablePrint.SendStringToPrinter(ptrn, sb.ToString()))
                    {
                        return common.GetModuleFromJobjet(total,true, "打印成功");
                    }
                    //string result = BarCodeLablePrint.IPPrint();
                }
                else if (pagecount == 2)
                {
                    bool resut = BarCodeLablePrint.SendStringToPrinter(ptrn, sb.ToString());
                    resut = BarCodeLablePrint.SendStringToPrinter(ptrn, sb.ToString());
                    return common.GetModuleFromJobjet(total,resut, resut ? "打印成功" : "打印失败");

                }
                return common.GetModuleFromJobjet(total,false, "打印失败");
            }
        }
        #endregion
        ...
        /// 找到delete.json,读取delete.json的数据,循环数据,找到本地有没有相同的文件名的json 文件,没有就直接移除delete.json的订单号,进行下一个循环,有的话就提示,入库,外箱,内箱确认,电检有多少条记录记录,并返回
        /// <returns></returns>
        public JObject DisplayOrdernum()
        {
            ...
            if (System.IO.File.Exists(@"D:\MES_Data\TemDate\OrderSequence\delete.json") == true)//判断有没有delete.json,没有就返回null
            {
                ...
                return common.GetModuleFromJarray(messagejson);
            }
            return null;
        }
        ...
        /// <param name="ordernum">订单号</param>
        /// <returns></returns>
        public JObject DeleteMoudleRuleAsync([System.Web.Http.FromBody]JObject data)
        {
            string ordernum = data["ordernum"].ToString();
            string UserName = data["UserName"].ToString();
            JObject message = new JObject();
            ...
            if (cab.Count != 0)
            {
                return common.GetModuleFromJobjet(message,false, "订单已有校正记录，不能删除");
            }
            var current = common.GetCurrentwarehousList(ordernum);
            var warehoueout = current.Count(c => c.IsOut == true);
            if (warehoueout > 0)
            {
                return common.GetModuleFromJobjet(message,false, "订单已有出库信息，不能删除");
            }
            var bac = db.BarCodes.Where(c => c.OrderNum == ordernum).ToList();//修改BarCodes表里的模组号,修改为null
            bac.ForEach(c => c.ModuleGroupNum = null);
            ...
            if (appercount > 0)
            {
                var appper = db.Appearance.Where(c => c.OrderNum == ordernum && c.OQCCheckFT != null);
                var barcode = db.BarCodes.Where(c => c.OrderNum == ordernum);
                barcode.ForEachAsync(c => c.ModuleGroupNum = null);
                db.Appearance.RemoveRange(appper);
            ...
            {
                //填写日志
                UserOperateLog log = new UserOperateLog() { Operator = UserName, OperateDT = DateTime.Now, OperateRecord = "订单模组号规则删除：订单号为" + ordernum };
                db.UserOperateLog.Add(log);
                db.SaveChanges();
                return common.GetModuleFromJobjet(message,true,"");
            }
            else
            {
                return common.GetModuleFromJobjet(message,false, "操作数据失败");
            }
        }
        ...
        /// 找到delete.json,删除前端传过来的订单号
        /// <param name="ordernum">订单</param>
        public void CancelDelete([System.Web.Http.FromBody]JObject data)
        {
            string ordernum = data["ordernum"].ToString();
            var deletejson = System.IO.File.ReadAllText(@"D:\MES_Data\TemDate\OrderSequence\delete.json");
            var jarray = JsonConvert.DeserializeObject<JArray>(deletejson).ToList();//读取文件
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject DisplayTotalModule([System.Web.Http.FromBody]JObject data)
        {
            string ordernum = data["ordernum"].ToString();
            string type = data["type"].ToString();//默认是模组
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject BatchUpdatTotalModule([System.Web.Http.FromBody]JObject data)
        {
            string ListMessage = data["ListMessage"].ToString();
            string UserName = data["UserName"].ToString();
            JArray value = JsonConvert.DeserializeObject<JArray>(ListMessage);
            JObject result = new JObject();
            var count = value.Count();
            ...
            if (temp2.Count != 0)
            {
                return common.GetModuleFromJobjet(result,false, "模组号" + string.Join(",", temp2.Select(c => c.Key)) + "重复");
            }
            for (int i = 0; i < count; i++)
            {
                var module = value[i]["module"].ToString();
                ...
                //填写日志
                UserOperateLog log = new UserOperateLog() { OperateDT = DateTime.Now, Operator = UserName, OperateRecord = "批量修改模组号,修改后条码为" + value[i]["module"].ToString() };
                db.UserOperateLog.Add(log);
                db.SaveChangesAsync();
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject UpdateMuduleFromCompleteAppearances([System.Web.Http.FromBody]JObject data)
        {
            string barcode = data["barcode"].ToString();
            string modulenum = data["modulenum"].ToString();
            string UserName = data["UserName"].ToString();
            ...
            if (message != null)//判断信息是否为空,为空提示错误
            {
                JObject jobject = common.UpdateTotalModule(barcode, modulenum, UserName);
                if (jobject["pass"].ToString() == "True")
                {
                    return common.GetModuleFromJobjet(result,true,"");
                }
                else
                {
                    return common.GetModuleFromJobjet(result,false, jobject["mes"].ToString());
                }
            }
            return common.GetModuleFromJobjet(result,false, "此条码不符合修改条件");
        }
        #endregion
    }
