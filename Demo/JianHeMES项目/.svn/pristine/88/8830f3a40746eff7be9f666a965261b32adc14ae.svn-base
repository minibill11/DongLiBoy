...
namespace JianHeMES.Controllers
{
    public class TipsController : Controller
    {
        ...
        public JArray Equipment_RepairbillQuery_TecDepar()
        {
            ...
            var repairbill = db.EquipmentRepairbill.Where(c => c.DeparAssessor != null && c.TecDepar_opinion == null).ToList();
        ...
        public JArray Equipment_RepairbillQuery_Purchasing()
        {
            ...
            var pubrch = db.EquipmentRepairbill.Where(c => c.MEName != null && c.Needto == true && c.Purchasing_opinion == null);
        ...
        public JArray Equipment_RepairbillQuery_MainName()
        {
            ...
            var Maintenan = db.EquipmentRepairbill.Where(c => c.MEName != null && c.Needto == true && c.OpinAssessor != null && c.MainName == null && c.UserDepartment == dep
            ).ToList();
        ...
        public JArray Equipment_RepairbillQuery_TcConfirmName()
        {
            ...
            {
                var tcafter = db.EquipmentRepairbill.Where(c => c.MEName != null && c.TcConfirmName == null && c.Needto == false).ToList();
            ...
            if (db.EquipmentRepairbill.Count(c => c.MEName != null && c.Needto == true && c.OpinAssessor != null && c.MainName != null) > 0)
            {
        ...
        public JArray Equipment_RepairbillQuery_AfterMain()
        {
            ...
            if (db.EquipmentRepairbill.Count(c => c.MEName != null && c.Needto == false) > 0)
            {
                var tcconfir = db.EquipmentRepairbill.Where(c => c.MEName != null && c.Needto == false && c.AfterMain == null && c.UserDepartment == dep).ToList();
    ...
    public class Tips_ApiController : System.Web.Http.ApiController
    {
        private ApplicationDbContext db = new ApplicationDbContext();
        ...
        public JObject Index([System.Web.Http.FromBody]JObject data)
        {
            ...
            {
                //小样报告
                if (item.Matters == "小样报告确认审核") result.Add("小样报告确认审核", SmallSampleNotAssessQuery().Count());
                if (item.Matters == "小样报告确认核准") result.Add("小样报告确认核准", SmallSampleNotApprovedQuery().Count());
                ////工序产能
                //if (item.Matters == "工序产能确认审核") result.Add("工序产能确认审核", Process_CapacityNotExaminanQuery().Count());
                //if (item.Matters == "工序产能确认批准") result.Add("工序产能确认批准", Process_CapacityNotApproverQuery().Count());
                //if (item.Matters == "工序产能确认受控") result.Add("工序产能确认受控", Process_CapacityNotControlledQuery().Count());
                //设备管理
                if (item.Matters == "设备报修单审核") result.Add("设备报修单审核", Equipment_Repairbill_AssessorQuery().Count());

                if (item.Matters == "报修单采购批准") result.Add("报修单采购批准", Equipment_Repairbill_ApproveQuery().Count());
                if (item.Matters == "报修单技术部意见批准") result.Add("报修单技术部意见批准", Equipment_Repairbill_ApproveQuery1().Count());
                if (item.Matters == "故障报修单批准") result.Add("故障报修单批准", Equipment_Repairbill_ApproveQuery2().Count());
                if (item.Matters == "技术部故障报修单批准") result.Add("技术部故障报修单批准", Equipment_Repairbill_ApproveQuery3().Count());

                if (item.Matters == "设备报修单技术部意见") result.Add("设备报修单技术部意见", Equipment_RepairbillQuery_TecDepar().Count());
                if (item.Matters == "设备报修单采购部意见") result.Add("设备报修单采购部意见", Equipment_RepairbillQuery_Purchasing().Count());
                if (item.Matters == "设备报修单维修人/厂家") result.Add("设备报修单维修人/厂家", Equipment_RepairbillQuery_MainName().Count());
                if (item.Matters == "设备报修单维修后确认/技术部") result.Add("设备报修单维修后确认/技术部", Equipment_RepairbillQuery_TcConfirmName().Count());
                if (item.Matters == "设备报修单维修后确认/维修需要部门") result.Add("设备报修单维修后确认/维修需要部门", Equipment_RepairbillQuery_AfterMain().Count());

               // if (item.Matters == "设备点检未确认点检保养") result.Add("设备点检未确认点检保养", Equipment_TallyQuery().Count());

                if (item.Matters == "未审核的质量指标达成状况统计表") result.Add("未审核的质量指标达成状况统计表", Equipment_QualitytargetQuery().Count());
                if (item.Matters == "未批准的质量指标达成状况统计表") result.Add("未批准的质量指标达成状况统计表", Equipment_ApprovetQuery().Count());
                if (item.Matters == "技术部未审核的安全库存清单") result.Add("技术部未审核的安全库存清单", Equipment_SafetyTec_AssessorQuery().Count());
                if (item.Matters == "MC部未审核的安全库存清单") result.Add("MC部未审核的安全库存清单", Equipment_SafetyAssessorQuery().Count());
                if (item.Matters == "工厂厂长未批准的安全库存清单") result.Add("工厂厂长未批准的安全库存清单", Equipment_SafetyApproveQuery().Count());
            }
        ...
        public JArray Equipment_Repairbill_AssessorQuery()
        {
            ...
            {
                dep = auth.Department == "智造部" ? "技术部" : auth.Department;
            }
            var maintenance = db.EquipmentRepairbill.Where(c => c.RepairName != null && c.DeparAssessor == null && c.UserDepartment == dep).ToList();
            foreach (var item in maintenance)//故障审核
            {
        ...
        #region----输出所有提示项名清单、输出用户所有提示项清单、输出提示项所有用户清单
        /// <summary>
        /// 输出所有提示项名清单
        /// </summary>
        /// <returns></returns>
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject UserTipsList1(int userId)
        {
            JArray result = new JArray();
            var tipsList = db.UserNeedsPrompt.Where(c => c.UserNum == userId).Select(c => new { c.id, c.UserName, c.UserNum, c.Matters });
            result.Add(tipsList);
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject UserTipsList2(string matter)
        {
            JArray result = new JArray();
            var tipsList = db.UserNeedsPrompt.Where(c => c.Matters == matter).Select(c => new { c.id, c.UserName, c.UserNum, c.Matters });
            result.Add(tipsList);
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject GetUserNameUserId()
        {
            JArray result = new JArray();
            var result1 = db.Users.Select(c => new { c.UserName, c.UserNum });
            result.Add(result1);
            return common.GetModuleFromJarray(result);
        }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject GetRoleName()
        {
            JArray result = new JArray();
            var result1 = db.UserRolelistTable.Select(c => c.Discription).Distinct();
            result.Add(result1);
            return common.GetModuleFromJarray(result);
        }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject TipsItemManagement_add([System.Web.Http.FromBody]JObject data)//添加
        {
            JObject result = new JObject();
            UserInformationTips tipsItem = JsonConvert.DeserializeObject<UserInformationTips>(JsonConvert.SerializeObject(data));
            //检查添加的提示项名称是否在权限名称列表中
            var count = db.UserRolelistTable.Count(c => c.RolesName == tipsItem.Matters);
            if (count > 0) return common.GetModuleFromJobjet(result, false, "提示项名称不权限名之中，请确认！");
            else
            {
                int tipsItemCount = db.UserInformationTips.Count(c => c.Matters == tipsItem.Matters);
                if (tipsItemCount > 0)
                {
                    return common.GetModuleFromJobjet(result, false, tipsItem.Matters + "已经存在!");
                }
                else
                {
                    db.UserInformationTips.Add(tipsItem);
                    db.SaveChanges();
                    return common.GetModuleFromJobjet(result, true,"提示项“" + tipsItem.Matters + "”添加成功。");
                }
            }
        }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject TipsItemManagement_modify([System.Web.Http.FromBody]JObject data)//修改
        {
            JObject result = new JObject();
            UserInformationTips tipsItem = JsonConvert.DeserializeObject<UserInformationTips>(JsonConvert.SerializeObject(data));
            AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
            ...
            if (isExist > 0)
            {
                ...
                db.UserOperateLog.Add(operaterecord);
                int count = db.SaveChanges();
                if (count > 0) return common.GetModuleFromJobjet(result, true, "修改提示项名称：" + originMatter + "==>" + tipsItem.Matters + "修改成功。");
                else return common.GetModuleFromJobjet(result, false, "修改失败");
            }
            else return common.GetModuleFromJobjet(result, false, "提示项名称不权限名之中，请确认！");
        }

        [HttpPost]
        [ApiAuthorize]
        public JObject TipsItemManagement_delete([System.Web.Http.FromBody]JObject data)//删除
        {
            JObject result = new JObject();
            AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
            var jsonStr = JsonConvert.SerializeObject(data);
            ...
            var tipsItem = db.UserInformationTips.Find(id);
            var userNeedsPromptRecordCount = db.UserNeedsPrompt.Count(c => c.Matters == tipsItem.Matters);
            if (userNeedsPromptRecordCount > 0) return common.GetModuleFromJobjet(result, false, tipsItem.Matters + "已经授权了" + userNeedsPromptRecordCount + "个用户提示信息，如要删除" + tipsItem.Matters + "提示项，需要先解除相应的授权。");
            else
            {
                db.UserInformationTips.Remove(tipsItem);
                //记录删除动作记录在UserOperateLog中
                ...
                db.UserOperateLog.Add(operaterecord);
                var count = db.SaveChanges();
                if (count > 0) return common.GetModuleFromJobjet(result, true, "删除" + tipsItem.Matters + "提示项成功。");
            }
            return common.GetModuleFromJobjet(result, false, "删除" + tipsItem.Matters + "提示项失败");
        }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject UserTipsItem_add([System.Web.Http.FromBody]JObject data)
        {
            JObject result = new JObject();
            UserNeedsPrompt tipsItem = JsonConvert.DeserializeObject<UserNeedsPrompt>(JsonConvert.SerializeObject(data));
            var isexist = db.UserNeedsPrompt.Count(c => c.Matters == tipsItem.Matters && c.UserName == tipsItem.UserName);
            if (isexist > 0) return common.GetModuleFromJobjet(result, false, tipsItem.UserName + "已有" + tipsItem.Matters + "权限");
            else
            {
                db.UserNeedsPrompt.Add(tipsItem);
                var count = db.SaveChanges();
                if (count > 0) return common.GetModuleFromJobjet(result, true, "保存成功");
                else return common.GetModuleFromJobjet(result, false, "保存失败");
            };
        }

        [HttpPost]
        [ApiAuthorize]
        public JObject UserTipsItem_delete([System.Web.Http.FromBody]JObject data)
        {
            JObject result = new JObject();
            AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
            var jsonStr = JsonConvert.SerializeObject(data);
            ...
            db.UserOperateLog.Add(operaterecord);
            var count = db.SaveChanges();
            if (count > 0) return common.GetModuleFromJobjet(result, true, "删除" + record.UserName + "的" + record.Matters + "提示项成功。");
           else return common.GetModuleFromJobjet(result, false, "删除" + record.UserName + "的" + record.Matters + "提示项失败。");
        }
