...
namespace JianHeMES.Controllers
{
  public class PersonnelController : Controller
  {
    ...
    public ActionResult Personal_Daily()
    {
      return View();
    }
    ...
    public async Task<ActionResult> getjsondata(DateTime date)
    {
      ...
      foreach (var item in departmentlist1)
      {
        ...
        if (recordCount > 0) //有记录
        {
          ...
          obj.Aurhorized_personnel = record.Aurhorized_personnel;//编制人数
          obj.Need_personnel = record.Need_personnel;//刚需人数
          obj.Today_dutynum = record.Today_dutynum;//今日在岗人数
          obj.Engineering = record.Engineering;//来惠工程人数
          obj.Employees_personnel = record.Employees_personnel;//当日正式工人数
          obj.Workers_personnel = record.Workers_personnel;//当日劳务工人数
          obj.Today_on_board_employees = record.Today_on_board_employees; //当日入职正式工人数
          obj.Today_on_board_workers = record.Today_on_board_workers; //当日入职劳务工人数
          ...
          obj.Interview = record.Interview;//预填表人数
          obj.Date = record.Date.ToString();
          obj.Reporter = record.Reporter;
          obj.Resigned_that_month = record.Resigned_that_month;//当月待离职正式工人数
          obj.Resigned_workers_that_month = record.Resigned_workers_that_month;//当月待离职劳务工人数
        }
        else  //当天无记录
        {
          ...
          obj.Today_dutynum = 0;//今日在岗人数
          obj.Engineering = 0;//来惠工程人数
        ...
        DateTime mon_begin_date = new DateTime(date.Year, date.Month, 1);//月初时间
                                                                         //本周离职离职正式工和劳务工人数
                                                                         //满七天正式工
        obj.Week_dimission_employees_over7days = Month_recordlist.Where(c => c.Date > monday && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Todoy_dimission_employees_over7days);
        //未满七天正式工
        obj.Week_dimission_employees_nvever_over7days = Month_recordlist.Where(c => c.Date > monday && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Todoy_dimission_employees_nvever_over7days);
        //劳务工
        obj.Week_dimission_workers = Month_recordlist.Where(c => c.Date > monday && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Todoy_dimission_workers);
        ...
        //本月离职正式工和劳务工人数
        obj.Month_dimission_employees = Month_recordlist.Where(c => c.Date > mon_begin_date && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Todoy_dimission_employees);//貌似没用到（正式工）
    ...
    {
      string file1 = "/MES_Data/Personnel_Files/人员动态表/" + date.Year + "/" + department + "/" + date.Year + "-" + date.Month.ToString("00") + "-" + date.Day.ToString("00") + department + "人员动态.xls";
      string file2 = "/MES_Data/Personnel_Files/人员动态表/" + date.Year + "/" + department + "/" + date.Year + "-" + date.Month.ToString("00") + "-" + date.Day.ToString("00") + department + "人员动态.xlsx";
    ...
    public async Task<ActionResult> Create([Bind(Include = "Id,Department,Principal,Aurhorized_personnel,Need_personnel,Employees_personnel,Workers_personnel,Today_on_board_employees,Today_on_board_workers,Interview,Todoy_dimission_employees,Todoy_dimission_employees_over7days,Todoy_dimission_employees_nvever_over7days,Todoy_dimission_workers,Resigned_that_month,Resigned_workers_that_month,Date,Reporter,Today_dutynum,Engineering")] Personnel_daily personnel_daily)
    {
      DateTime date = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day);
    ...
    public ActionResult Archit_depar(DateTime date)
    {
      ...
      var datelist = db.Personnel_Architecture.Where(c => c.ExecutionTime == exdate).Select(c => new { c.Id, c.Department, c.Principal, c.ClerkName, c.Aurhorized_personnel, c.Need_personnel, c.ExecutionTime }).ToList();
    ...
    //根据‘用户名（文员）’显示对应的数据
    public ActionResult QueryClerk(string clerkName)
    {
      DateTime date = DateTime.Now;
      var exdate = db.Personnel_Architecture.Where(c => c.ExecutionTime <= date).Max(c => c.ExecutionTime);
      var clerklist = db.Personnel_Architecture.Where(c => c.ExecutionTime == exdate && c.ClerkName == clerkName).Select(c => new { c.Department, c.Principal, c.Aurhorized_personnel, c.Need_personnel }).ToList();
      return Content(JsonConvert.SerializeObject(clerklist));
    }
  ...
  public class Personnel_ApiController : System.Web.Http.ApiController
  {
    private ApplicationDbContext db = new ApplicationDbContext();
    ...
    [HttpPost]
    [ApiAuthorize]
    public JObject Archit_depar([System.Web.Http.FromBody]JObject data)
    {
      ...
      var exdate = db.Personnel_Architecture.Where(c => c.ExecutionTime <= dateTime).Max(c => c.ExecutionTime);
      var datelist = db.Personnel_Architecture.Where(c => c.ExecutionTime == exdate).Select(c => new { c.Id, c.Department, c.Principal, c.ClerkName, c.Aurhorized_personnel, c.Need_personnel, c.ExecutionTime }).ToList();
    ...
    [HttpPost]
    [ApiAuthorize]
    public JObject EditArchitecture([System.Web.Http.FromBody]JObject data)
    {
      JObject result = new JObject();
      AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
      Personnel_Architecture architeList = JsonConvert.DeserializeObject<Personnel_Architecture>(JsonConvert.SerializeObject(data));
      if (architeList != null && architeList.ExecutionTime != null && architeList.Department != null && architeList.Principal != null && architeList.Id != 0)
      {
        architeList.ModifyTime = DateTime.Now;
        architeList.ModifyName = auth.UserName;
        db.Entry(architeList).State = EntityState.Modified;
        var savecount = db.SaveChanges();
        if (savecount > 0)
        {
    ...
    [HttpPost]
    [ApiAuthorize]
    public JObject Createarchitecture([System.Web.Http.FromBody]JObject data)
    {
      JArray result = new JArray();
      JObject repat = new JObject();
      AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
      var jsonStr = JsonConvert.SerializeObject(data);
      var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
      List<Personnel_Architecture> inputList = obj.inputList;
      if (inputList != null)
      {
        foreach (var item in inputList)
        {
    ...
    [HttpPost]
    [ApiAuthorize]
    public JObject QueryClerk([System.Web.Http.FromBody]JObject data)
    {
      var jsonStr = JsonConvert.SerializeObject(data);
      var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
      DateTime dateTime = DateTime.Now;
      string clerkName = obj.clerkName;
      var exdate = db.Personnel_Architecture.Where(c => c.ExecutionTime <= dateTime).Max(c => c.ExecutionTime);
      var clerklist = db.Personnel_Architecture.Where(c => c.ExecutionTime == exdate && c.ClerkName == clerkName).Select(c => new { c.Department, c.Principal, c.Aurhorized_personnel, c.Need_personnel }).ToList();
      JArray result = new JArray
            {
    ...
    [HttpPost]
    [ApiAuthorize]
    public JObject PersonnelData([System.Web.Http.FromBody]JObject data)
    {
      JArray result = new JArray();
      var jsonStr = JsonConvert.SerializeObject(data);
      var obj_date = JsonConvert.DeserializeObject<dynamic>(jsonStr);
      DateTime date = obj_date.date;//时间
      if (date == null)
      {
        date = DateTime.Now;
      }
      ...
      var weeknum = date.DayOfWeek.ToString();//得到date对应是星期几
      List<Personnel_Architecture> departmentlist1 = common.CompanyDatetime(date);
      List<string> departmentlist = departmentlist1.Select(c => c.Department).ToList();
      //取出本月数据
      var Month_recordlist = db.Personnel_daily.Where(c => c.Date.Value.Year == dateV.Year && c.Date.Value.Month == dateV.Month).ToList();
      //修正确定monday的日期
      switch (weeknum)
      {
        case "Monday": //星期一
      ...
      //取出本周数据
      var this_week_recordlist = db.Personnel_daily.Where(c => c.Date > monday && c.Date < tomorrow).ToList();
      //如果月和周数据都为空，则返回“无记录”
      if (Month_recordlist.Count == 0 && this_week_recordlist.Count() == 0 || date.Date > sunday || date < new DateTime(2018, 12, 3))
      {
        return common.GetModuleFromJarray(result, false, "无记录");
      }
      ...
      foreach (var item in departmentlist1)
      {
        ...
        if (recordCount > 0) //有记录
        {
          recordList.Add("Id", record.Id);
          recordList.Add("Department", record.Department);
          recordList.Add("Principal", record.Principal);
          recordList.Add("Aurhorized_personnel", record.Aurhorized_personnel);//编制人数
          recordList.Add("Need_personnel", record.Need_personnel);//刚需人数
          recordList.Add("Today_dutynum", record.Today_dutynum);//今日在岗人数
          recordList.Add("Engineering", record.Engineering);//来惠工程人数
          recordList.Add("Employees_personnel", record.Employees_personnel);//当日正式工人数
          recordList.Add("Workers_personnel", record.Workers_personnel);//当日劳务工人数
          recordList.Add("Today_on_board_employees", record.Today_on_board_employees); //当日入职正式工人数
          recordList.Add("Today_on_board_workers", record.Today_on_board_workers); //当日入职劳务工人数
          recordList.Add("Huizong", record.Employees_personnel + record.Workers_personnel + record.Today_on_board_employees + record.Today_on_board_workers);
          recordList.Add("Dangqian_que", record.Need_personnel - (record.Employees_personnel + record.Workers_personnel + record.Today_on_board_employees + record.Today_on_board_workers));
          recordList.Add("Interview", record.Interview);//预填表人数
          recordList.Add("Date", record.Date.ToString());
          recordList.Add("Reporter", record.Reporter);
          recordList.Add("Resigned_that_month", record.Resigned_that_month);//当月待离职正式工人数
          recordList.Add("Resigned_workers_that_month", record.Resigned_workers_that_month);//当月待离职劳务工人数
        }
        else  //当天无记录
        {
          var lastrecord = Month_recordlist.Where(c => c.Department == item.Department && c.Date < date).OrderByDescending(c => c.Id).Count() > 0 ? Month_recordlist.Where(c => c.Department == item.Department && c.Date < date).OrderByDescending(c => c.Id).FirstOrDefault() : db.Personnel_daily.Where(c => c.Department == item.Department && c.Date < date).OrderByDescending(c => c.Id).FirstOrDefault();
          recordList.Add("Id", 0);
          recordList.Add("Department", lastrecord == null ? item.Department : lastrecord.Department);
          recordList.Add("Principal", item.Principal);
          recordList.Add("Aurhorized_personnel", item.Aurhorized_personnel);
          recordList.Add("Need_personnel", item.Need_personnel);
          recordList.Add("Employees_personnel", 0);
          recordList.Add("Workers_personnel", 0);
          recordList.Add("Today_dutynum", 0);//今日在岗人数
          recordList.Add("Engineering", 0);//来惠工程人数
          recordList.Add("Today_on_board_employees", 0);
          recordList.Add("Today_on_board_workers", 0);
          recordList.Add("Huizong", 0);
          recordList.Add("Dangqian_que", 0);
          recordList.Add("Interview", 0);
          recordList.Add("Date", "");
          recordList.Add("Reporter", "未上报");
          recordList.Add("Resigned_that_month", 0);
          recordList.Add("Resigned_workers_that_month", 0);
        }
        DateTime mon_begin_date = new DateTime(date.Year, date.Month, 1);//月初时间

        //本周离职离职正式工和劳务工人数
        //满七天正式工
        recordList.Add("Week_dimission_employees_over7days", Month_recordlist.Where(c => c.Date > monday && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Todoy_dimission_employees_over7days));
        //未满七天正式工
        recordList.Add("Week_dimission_employees_nvever_over7days", Month_recordlist.Where(c => c.Date > monday && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Todoy_dimission_employees_nvever_over7days));
        //劳务工
        recordList.Add("Week_dimission_workers", Month_recordlist.Where(c => c.Date > monday && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Todoy_dimission_workers));

        //本月入职正式工和劳务工人数
        recordList.Add("Month_on_board_employees", Month_recordlist.Where(c => c.Date > mon_begin_date && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Today_on_board_employees));
        recordList.Add("Month_on_board_workers", Month_recordlist.Where(c => c.Date > mon_begin_date && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Today_on_board_workers));
        //本月离职正式工和劳务工人数
        recordList.Add("Month_dimission_employees", Month_recordlist.Where(c => c.Date > mon_begin_date && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Todoy_dimission_employees));//貌似没用到
        recordList.Add("Month_dimission_employees_over7days", Month_recordlist.Where(c => c.Date > mon_begin_date && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Todoy_dimission_employees_over7days));
        recordList.Add("Month_dimission_employees_nvever_over7days", Month_recordlist.Where(c => c.Date > mon_begin_date && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Todoy_dimission_employees_nvever_over7days));
        recordList.Add("Month_dimission_workers", Month_recordlist.Where(c => c.Date > mon_begin_date && c.Date < tomorrow && c.Department == item.Department).Sum(c => c.Todoy_dimission_workers));
        ...
        {
          case "Monday":
            week_recordlist = db.Personnel_daily.Where(c => c.Date > dateV && c.Date < tomorrow && c.Department == item.Department).OrderBy(c => c.Date).ToList();
            if (week_recordlist.Count == 0)
            {
              recordList.Add("Mon_workers", 0);
              recordList.Add("Mon_employees", 0);
              recordList.Add("Week_total_employees", 0);
              recordList.Add("Week_total_workers", 0);
            }
            else
            {
              recordList.Add("Mon_workers", record == null ? 0 : record.Today_on_board_workers);
              recordList.Add("Mon_employees", record == null ? 0 : record.Today_on_board_employees);
              //本周入职、离职正式工和劳务工人数
              recordList.Add("Week_total_employees", record == null ? 0 : record.Today_on_board_employees);
              recordList.Add("Week_total_workers", record == null ? 0 : record.Today_on_board_workers);
            }
            recordList.Add("Tues_workers", 0);
            recordList.Add("Tues_employees", 0);
            recordList.Add("Wednes_workers", 0);
            recordList.Add("Wednes_employees", 0);
            recordList.Add("Thurs_workers", 0);
            recordList.Add("Thurs_employees", 0);
            recordList.Add("Fri_workers", 0);
            recordList.Add("Fri_employees", 0);
            break;
          case "Tuesday":
            monday = dateV.AddDays(-1);
            week_recordlist = db.Personnel_daily.Where(c => c.Date > monday && c.Date < tomorrow && c.Department == item.Department).OrderBy(c => c.Date).ToList();
            if (week_recordlist.Count == 0)
            {
              recordList.Add("Mon_workers", 0);
              recordList.Add("Mon_employees", 0);
            ...
            {
              recordList.Add("Mon_workers", week_recordlist.FirstOrDefault().Date.Value.DayOfWeek.ToString() == "Monday" ? week_recordlist.FirstOrDefault().Today_on_board_workers : 0);
              recordList.Add("Mon_employees", week_recordlist.FirstOrDefault().Date.Value.DayOfWeek.ToString() == "Monday" ? week_recordlist.FirstOrDefault().Today_on_board_employees : 0);
              //本周入职、离职正式工和劳务工人数
              recordList.Add("Week_total_employees", week_recordlist.Sum(c => c.Today_on_board_employees));
              recordList.Add("Week_total_workers", week_recordlist.Sum(c => c.Today_on_board_workers));
              recordList.Add("Tues_workers", record == null ? 0 : record.Today_on_board_workers);
              recordList.Add("Tues_employees", record == null ? 0 : record.Today_on_board_employees);
            }
            recordList.Add("Wednes_workers", 0);
            recordList.Add("Wednes_employees", 0);
            recordList.Add("Thurs_workers", 0);
          ...
          case "Wednesday": //星期三 
            ...
            else
            {
              recordList.Add("Mon_workers", week_recordlist.FirstOrDefault().Date.Value.DayOfWeek.ToString() == "Monday" ? week_recordlist.FirstOrDefault().Today_on_board_workers : 0);
              recordList.Add("Mon_employees", week_recordlist.FirstOrDefault().Date.Value.DayOfWeek.ToString() == "Monday" ? week_recordlist.FirstOrDefault().Today_on_board_employees : 0);
              recordList.Add("Tues_workers", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Tuesday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Tuesday").FirstOrDefault().Today_on_board_workers);
              recordList.Add("Tues_employees", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Tuesday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Tuesday").FirstOrDefault().Today_on_board_employees);
              //本周入职、离职正式工和劳务工人数
              recordList.Add("Week_total_employees", week_recordlist.Sum(c => c.Today_on_board_employees));
              recordList.Add("Week_total_workers", week_recordlist.Sum(c => c.Today_on_board_workers));
            }
            recordList.Add("Wednes_workers", record == null ? 0 : record.Today_on_board_workers);
            recordList.Add("Wednes_employees", record == null ? 0 : record.Today_on_board_employees);
            recordList.Add("Thurs_workers", 0);
            recordList.Add("Thurs_employees", 0);
            recordList.Add("Fri_workers", 0);
            recordList.Add("Fri_employees", 0);
          ...
          case "Thursday": //星期四
            ...
            else
            {
              ...
              recordList.Add("Tues_workers", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Tuesday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Tuesday").FirstOrDefault().Today_on_board_workers);
              recordList.Add("Tues_employees", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Tuesday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Tuesday").FirstOrDefault().Today_on_board_employees);
              recordList.Add("Wednes_workers", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Wednesday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Wednesday").FirstOrDefault().Today_on_board_workers);
              recordList.Add("Wednes_employees", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Wednesday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Wednesday").FirstOrDefault().Today_on_board_employees);
              recordList.Add("Thurs_workers", record == null ? 0 : record.Today_on_board_workers);
              recordList.Add("Thurs_employees", record == null ? 0 : record.Today_on_board_employees);
              //本周入职、离职正式工和劳务工人数
              recordList.Add("Week_total_employees", week_recordlist.Sum(c => c.Today_on_board_employees));
              recordList.Add("Week_total_workers", week_recordlist.Sum(c => c.Today_on_board_workers));
            }
          ...
          case "Friday": //星期五
            ...
            else
            {
              ...
              recordList.Add("Wednes_workers", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Wednesday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Wednesday").FirstOrDefault().Today_on_board_workers);
              recordList.Add("Wednes_employees", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Wednesday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Wednesday").FirstOrDefault().Today_on_board_employees);
              recordList.Add("Thurs_workers", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Thursday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Thursday").FirstOrDefault().Today_on_board_workers);
              recordList.Add("Thurs_employees", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Thursday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Thursday").FirstOrDefault().Today_on_board_employees);
              recordList.Add("Fri_workers", record == null ? 0 : record.Today_on_board_workers);
              recordList.Add("Fri_employees", record == null ? 0 : record.Today_on_board_employees);
              //本周入职、离职正式工和劳务工人数
              recordList.Add("Week_total_employees", week_recordlist.Sum(c => c.Today_on_board_employees));
              recordList.Add("Week_total_workers", week_recordlist.Sum(c => c.Today_on_board_workers));
            }
          ...
          case "Saturday":
            ...
            else
            {
              ...
              recordList.Add("Thurs_workers", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Thursday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Thursday").FirstOrDefault().Today_on_board_workers);
              recordList.Add("Thurs_employees", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Thursday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Thursday").FirstOrDefault().Today_on_board_employees);
              recordList.Add("Fri_workers", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Friday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Friday").FirstOrDefault().Today_on_board_workers);
              recordList.Add("Fri_employees", week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Friday").Count() == 0 ? 0 : week_recordlist.Where(c => c.Date.Value.DayOfWeek.ToString() == "Friday").FirstOrDefault().Today_on_board_employees);
              //本周入职、离职正式工和劳务工人数
              recordList.Add("Week_total_employees", week_recordlist.Sum(c => c.Today_on_board_employees));
              recordList.Add("Week_total_workers", week_recordlist.Sum(c => c.Today_on_board_workers));
            }
      ...
      if (result.Count > 0)
      {
        ...
        return common.GetModuleFromJarray(result, false, "没有记录");
    }
    ...
    public JObject Create([System.Web.Http.FromBody]JObject data)
    {
      JObject result = new JObject();
      AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
      Personnel_daily personnel_daily = JsonConvert.DeserializeObject<Personnel_daily>(JsonConvert.SerializeObject(data));
      DateTime date = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day);
      if (personnel_daily != null && personnel_daily.Date != null)
      {
        var recordCount = db.Personnel_daily.Where(c => c.Date.Value.Year == date.Year && c.Date.Value.Month == date.Month && c.Date.Value.Day == date.Day && c.Department == personnel_daily.Department).Count();
        if (recordCount > 0)
        {
          return common.GetModuleFromJobjet(result, false, "此部门的人事日报已经上报过");
        }
        personnel_daily.Interview = 0;
        personnel_daily.Date = DateTime.Now;
        personnel_daily.Reporter = auth.UserName;
        db.Personnel_daily.Add(personnel_daily);
        int count = db.SaveChanges();
        if (count > 0)
        {
          return common.GetModuleFromJobjet(result, true, "提交成功");
        }
        else
        {
          return common.GetModuleFromJobjet(result, false, "提交失败");
        }
      }
      return common.GetModuleFromJobjet(result, false, "提交失败");
    }
    ...
    public JObject Edit([System.Web.Http.FromBody]JObject data)
    {
      ...
      AuthInfo auth = (AuthInfo)this.RequestContext.RouteData.Values["Authorization"];
      Personnel_daily personnel_daily = JsonConvert.DeserializeObject<Personnel_daily>(JsonConvert.SerializeObject(data));
      if (personnel_daily != null && personnel_daily.Id != 0)
      {
        personnel_daily.Date = DateTime.Now;
        personnel_daily.Reporter = auth.UserName;
        db.Entry(personnel_daily).State = EntityState.Modified;
    ...
    {
      JObject result = new JObject();
      DateTime dateTime = Convert.ToDateTime(HttpContext.Current.Request["dateTime"]);
      ...
      if (requests.Files.Count > 0)
      {
        for (int i = 0; i < requests.Files.Count; i++)
        {
          ...
          if (re)
          {
            return common.GetModuleFromJobjet(result, false, "<script>alert('您选择文件的文件类型不正确，请选择xls或xlsx类型文件！');history.go(-1);</script>");
          }
          string ReName = dateTime.Year + "-" + dateTime.Month.ToString("00") + "-" + dateTime.Day.ToString("00") + department + "人员动态";  //重命名文件
          if (Directory.Exists(@"D:\MES_Data\Personnel_Files\人员动态表\" + dateTime.Year + "\\" + department + "\\") == false)//如果不存在就创建订单文件夹
          {
        ...
        {
          return common.GetModuleFromJobjet(result, false, "上传失败,请重新选择文件");
        }
      }
      return common.GetModuleFromJobjet(result, false, "请选择文件");
    }
    ...
    [HttpPost]
    [ApiAuthorize]
    public JObject ExcelFile([System.Web.Http.FromBody]JObject data)
    {
      JObject result = new JObject();
      var jsonStr = JsonConvert.SerializeObject(data);
      var obj = JsonConvert.DeserializeObject<dynamic>(jsonStr);
      ...
      DateTime dateTime = obj.dateTime;
      if (department != null && dateTime != null)
      {
        string file1 = "/MES_Data/Personnel_Files/人员动态表/" + dateTime.Year + "/" + department + "/" + dateTime.Year + "-" + dateTime.Month.ToString("00") + "-" + dateTime.Day.ToString("00") + department + "人员动态.xls";
        string file2 = "/MES_Data/Personnel_Files/人员动态表/" + dateTime.Year + "/" + department + "/" + dateTime.Year + "-" + dateTime.Month.ToString("00") + "-" + dateTime.Day.ToString("00") + department + "人员动态.xlsx";
        if (Directory.Exists(@"D:\MES_Data\Personnel_Files\人员动态表\" + dateTime.Year + "\\" + department + "\\"))
        {
          List<FileInfo> fileInfos = commonality.GetAllFilesInDirectory(@"D:\MES_Data\Personnel_Files\人员动态表\" + dateTime.Year + "\\" + department + "\\");
          foreach (var it in fileInfos)
          {
            string path = @"/MES_Data/Personnel_Files/人员动态表/" + dateTime.Year + "/" + department + "/" + it;//组合出路径
            if (path == file2)
            {
              var filetype = path.Split('.');//将组合出来的路径以点分隔，方便下一步判断后缀
              if (filetype[1] == "xlsx")//后缀为jpg
              {
                result.Add("Data", path);
              }
            }
            else if (path == file1)//后缀为其他
            {
              var filetype = path.Split('.');//将组合出来的路径以点分隔，方便下一步判断后缀
              if (filetype[1] == "xls")//后缀为jpg
              {
                result.Add("Data", path);
              }
            }
