...
namespace JianHeMES.Controllers
{
    public class CalibrationRecordsController : Controller
    {
        ...
        public ActionResult CreateCal([Bind(Include = "ID,OrderNum,ModuleGroupNum,BarCodesNum,BeginCalibration,FinishCalibration,Normal,AbnormalDescription,CalibrationTime,CalibrationTimeSpan,Operator,RepetitionCalibration,RepetitionCalibrationCause")] CalibrationRecord calibrationRecord, string nuoBarCode = null)
        {
            //if (Session["User"] == null)
            //{
                ...
                else if (db.CalibrationRecord.Where(c => c.OrderNum == calibrationRecord.OrderNum && c.BarCodesNum == calibrationRecord.BarCodesNum && c.Normal == true && c.BeginCalibration != null && c.FinishCalibration != null).Count() > 0) //已经校正完成
                {
                    if (calibrationRecord.RepetitionCalibration == true)//重复校正已打钩
                    {
                        ...
                        if (!string.IsNullOrEmpty(nuoBarCode) && recount == 0)
                            comm.InsertRelation(nuoBarCode, calibrationRecord.BarCodesNum, calibrationRecord.OrderNum, "calibration");
