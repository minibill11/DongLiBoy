    ...
    public class Personnel_TurnoverrateController : Controller
    {
        ...
        public async Task<ActionResult> TurnoverrateYearlyLineChartData(int? Year)
        {
            ...
            foreach (var mouth in mouths)
            {
                ...
                foreach (var dep in department)
                {
                    ...
                    {
                        //月初人数 
                        var begin_day_of_month = Year_Allrecord.Where(c => c.Department == dep && c.Date.Value.Month == mouth).OrderBy(c => c.Date).FirstOrDefault().Employees_personnel + Year_Allrecord.Where(c => c.Department == dep && c.Date.Value.Month == mouth).OrderBy(c => c.Date).FirstOrDefault().Today_on_board_employees;
                        //月末人数
                        var end_day_of_month = Year_Allrecord.Where(c => c.Department == dep && c.Date.Value.Month == mouth).OrderByDescending(c => c.Date).FirstOrDefault().Employees_personnel + Year_Allrecord.Where(c => c.Department == dep && c.Date.Value.Month == mouth).OrderByDescending(c => c.Date).FirstOrDefault().Today_on_board_employees;
                        //平均数
                        decimal average = (begin_day_of_month + end_day_of_month) / 2;
                        average_sum = average_sum + average;
                        //离职人数总和
        ...
        public async Task<ActionResult> TurnoverrateYearly(string department,int? year)
        {
            ...
            foreach (var mouth in mouths)
            {
                ...
                //月初人数
                var begin_day_of_month = Year_Department_Allrecord.Where(c => c.Date.Value.Month == mouth).OrderBy(c => c.Date).FirstOrDefault().Employees_personnel + Year_Department_Allrecord.Where(c => c.Date.Value.Month == mouth).OrderBy(c => c.Date).FirstOrDefault().Today_on_board_employees;
