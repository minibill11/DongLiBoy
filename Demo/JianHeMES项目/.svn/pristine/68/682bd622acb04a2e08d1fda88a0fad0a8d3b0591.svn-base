...
namespace JianHeMES.Controllers
{
    public class Burn_inController : Controller
    {
        ...
        public ActionResult New_Index()
        {
            ...
            {
                return RedirectToAction("Login2", "Users", new { col = "Burn_in", act = "New_Index" });
            }
            return View();
        }
        public ActionResult New_Burn_in()
        {
            if (Session["User"] == null)
            {
                return RedirectToAction("Login2", "Users", new { col = "Burn_in", act = "New_Burn_in" });
            }
            return View();
        }
        ...
        public ActionResult NewIndex(string ordernum)
        {
            ...
            var info = db.Burn_in.Where(c => c.OrderNum == ordernum && (c.OldOrderNum == null || c.OldOrderNum == ordernum)).Select(c => new CommonController.TempIndex
            {
                BarCodesNum = c.BarCodesNum,
                EndTime = c.OQCCheckFT,
                Finish = c.OQCCheckFinish,
                StarTime = c.OQCCheckBT,
                OrderNum = c.OrderNum,
                Group = c.Group,
                Principal = c.OQCPrincipal,
                RepairCondition = c.RepairCondition,
                Abnormal = c.Burn_in_OQCCheckAbnormal,
                BurnInShelfNum = c.BurnInShelfNum,
            }).ToList();
        ...
        public ActionResult BurnInShelfQuery1(string OrderNum, string BurnInShelfNum)
        {
            JObject result = new JObject();
            JObject item = new JObject();
            JObject burnShelfNumList = new JObject();
        ...
        public ActionResult BurnInShelfQueryHistory1(string OrderNum, string BurnInShelfNum)
        {
            JObject result = new JObject();
            JObject item = new JObject();
            JObject burnShelfNumList = new JObject();
        ...
        public ActionResult BurnInShelfNumDoingList1()
        {
            var burninshelfs = db.Burn_in.Where(c => c.OQCCheckBT != null && c.OQCCheckFT == null && c.OQCCheckFinish == false && !string.IsNullOrEmpty(c.BurnInShelfNum)).Select(m => m.BurnInShelfNum).Distinct().ToList();
        ...
        private ActionResult BurnInShelfNumFinshingList1()
        {
            var burninshelfs = db.Burn_in.Where(c => c.OQCCheckBT != null && c.OQCCheckFT != null && c.OQCCheckFinish == true && !string.IsNullOrEmpty(c.BurnInShelfNum)).Select(m => m.BurnInShelfNum).Distinct().ToList();
            var items = new List<SelectListItem>();
            JArray result = new JArray();
        ...
        private ActionResult GetBurnInShelfOrderDoingList1()
        {
            var burnin_shelfsnum_orderlist = db.Burn_in.Where(c => c.OQCCheckFT == null && c.OQCCheckBT != null && c.OQCCheckFinish == false && c.BurnInShelfNum != null).Select(m => m.OrderNum).Distinct();
            JArray result = new JArray();
        ...
        private ActionResult GetBurnInShelfOrderFisishList1()
        {
            var burnin_shelfsnum_orderlist = db.Burn_in.Where(c => c.OQCCheckFT != null && c.OQCCheckBT != null && c.OQCCheckFinish == true).Select(m => m.OrderNum).Distinct();
            var items = new List<SelectListItem>();
            JArray result = new JArray();
        ...
        public async Task<ActionResult> Burn_in_B1(Burn_in burn_in, string nuoOrder = null, string nuoBarCode = null, string isnuo = null)
        {
            JObject result = new JObject();
            if (Session["User"] == null)
            {
            ...
            if (db.BarCodes.FirstOrDefault(u => u.BarCodesNum == burn_in.BarCodesNum) == null)
            {
                result.Add("mes", "模组条码不存在，请检查条码输入是否正确！");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Burn_in_F([Bind(Include = "Id,OrderNum,BurnInShelfNum,BarCodesNum,OQCCheckBT,OQCPrincipal,OQCCheckFT,OQCCheckTime,OQCCheckTimeSpan,Burn_in_OQCCheckAbnormal,RepairCondition,OQCCheckFinish,Department1,Group")] Burn_in burn_in)
        {
            if (Session["User"] == null)
            {
        ...
        private ActionResult GetOrderNumList1()
        {
            var ordernum = db.OrderMgm.OrderBy(m => m.OrderNum).Select(m => m.OrderNum).Distinct();
    ...
    public class Burn_in_ApiController : System.Web.Http.ApiController
    {
        private ApplicationDbContext db = new ApplicationDbContext();
        ...
        public JObject BurnInShelfQuery([System.Web.Http.FromBody]JObject data)
        {
            string OrderNum = data["OrderNum"].ToString();
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject BurnInShelfQueryHistory([System.Web.Http.FromBody]JObject data)
        {
            string OrderNum = data["OrderNum"].ToString();
            string BurnInShelfNum = data["BurnInShelfNum"].ToString();
            JObject result = new JObject();
        ...
        #region ---------------------------------------单个模组老化开始 去掉挪用

        [HttpPost]
        [ApiAuthorize]

        public JObject Burn_in_B([System.Web.Http.FromBody]JObject data)
        {
            ...
            {
                //此条码没有做过任何记录,直接创建新记录
                if (db.Burn_in.Count(u => u.BarCodesNum == BarCodesNum && (u.OldBarCodesNum == null || u.OldBarCodesNum == BarCodesNum)) == 0)//没有记录
                {
                    Burn_in burn_in = new Burn_in();
                    burn_in.OrderNum = OrderNum;
                    burn_in.BarCodesNum = BarCodesNum;
                    burn_in.OQCCheckBT = DateTime.Now;
                    burn_in.OQCPrincipal = UserName;
                    burn_in.OldBarCodesNum = burn_in.BarCodesNum;
                    burn_in.OldOrderNum = burn_in.OrderNum;
                    burn_in.Department1 = Department;
                    burn_in.Group = Group;
                    db.Burn_in.Add(burn_in);
                    db.SaveChangesAsync();
                    
                    return com.GetModuleFromJobjet(result,true, "老化成功");
                }
                ...
                {
                    //查找是否有正在老化中的条码 
                    var burn_in_list = db.Burn_in.Count(m => m.BarCodesNum == BarCodesNum && (m.OldBarCodesNum == null || m.OldBarCodesNum == BarCodesNum) && m.OQCCheckBT != null && m.OQCCheckFT == null);
                    //查找是否有正常完成老化的条码
                    var finshList = db.Burn_in.Count(m => m.BarCodesNum == BarCodesNum && (m.OldBarCodesNum == null || m.OldBarCodesNum == BarCodesNum) && m.OQCCheckFT != null && m.OQCCheckFinish == true);
                    if (burn_in_list != 0)
                    {
                        return com.GetModuleFromJobjet(result,false, "此模组已经开始老化，不能重复开始老化！");
                    }
                    else if (finshList != 0)
                    {
                        return com.GetModuleFromJobjet(result,false, "此模组已经完成老化，不能重复老化！");
                    }
                    else
                    {
                        Burn_in burn_in = new Burn_in();
        ...
        #region 老化异常录入前检查条码和订单
        [HttpPost]
        [ApiAuthorize]

        public JObject Get_Burn_in_Record([System.Web.Http.FromBody]JObject data)
        {
            string OrderNum = data["OrderNum"].ToString();
            string BarCodeNum = data["BarCodeNum"].ToString();

            Burn_in record = db.Burn_in.Where(m => m.OrderNum == OrderNum && m.BarCodesNum == BarCodeNum && (m.OldBarCodesNum == null || m.OldBarCodesNum == BarCodeNum) && m.OQCCheckBT != null && m.OQCCheckFT == null).FirstOrDefault();
            JObject recordData = new JObject();
            if (record != null)
            {
                recordData.Add("mes", "成功");
                recordData.Add("pass", true);
                recordData.Add("RecordId", record.Id);
                recordData.Add("RecordOrderNum", record.OrderNum);
                recordData.Add("RecordBarCodesNum", record.BarCodesNum);
                recordData.Add("RecordOQCCheckBT", record.OQCCheckBT.ToString());
                recordData.Add("RecordBurnInShelfNum", record.BurnInShelfNum);
                recordData.Add("recordOQCPrincipal", record.OQCPrincipal);
                recordData.Add("RecordBurn_in_OQCCheckAbnormal_old", record.Burn_in_OQCCheckAbnormal);

                return com.GetModuleFromJobjet(recordData);
            }
            else
            {
                recordData.Add("mes", "不存在正在进行老化的此模组");
                recordData.Add("pass", false);
                recordData.Add("RecordId", null);
                recordData.Add("RecordOrderNum", null);
                recordData.Add("RecordBarCodesNum", null);
                recordData.Add("RecordOQCCheckBT", null);
                recordData.Add("RecordBurnInShelfNum", null);
                recordData.Add("recordOQCPrincipal", null);
                recordData.Add("RecordBurn_in_OQCCheckAbnormal_old", null);
                return com.GetModuleFromJobjet(recordData);
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject AbnormalRecordInput([System.Web.Http.FromBody]JObject data)
        {
            int Id = int.Parse(data["Id"].ToString());
            string Burn_in_OQCCheckAbnormal = data["Burn_in_OQCCheckAbnormal"].ToString();
            JObject result = new JObject();
            ApplicationDbContext db = new ApplicationDbContext();
            ...
            if (burn_in == null)
            {
                return com.GetModuleFromJobjet(result,false, "找不到此条码信息");
            }
            else
            {
                burn_in.Burn_in_OQCCheckAbnormal = burn_in.Burn_in_OQCCheckAbnormal + Burn_in_OQCCheckAbnormal;
                ...
                db.Entry(burn_in).State = EntityState.Modified;
                db.SaveChanges();
                return com.GetModuleFromJobjet(result,true, "修改成功");
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject Burn_in_Batch_Begin([System.Web.Http.FromBody]JObject data)
        {
            string OrderNum = data["OrderNum"].ToString();
            string BurnInShelfNum = data["BurnInShelfNum"].ToString();
            string UserName = data["UserName"].ToString();
            ...
            foreach (var item in BarCodesNumList)
            {
                var count = db.Burn_in.Count(c => c.BarCodesNum == item.ToString() && (c.OldBarCodesNum == null || c.OldBarCodesNum == item.ToString()));
                if (count == 0)
                {
                    newRecord.BarCodesNum = item.ToString();
                    newRecord.OQCCheckBT = DateTime.Now;
                    newRecord.OQCPrincipal = UserName;
                    newRecord.BurnInShelfNum = BurnInShelfNum;
                    newRecord.OldOrderNum = OrderNum;
                    newRecord.OldBarCodesNum = item.ToString();
                ...
                else
                {
                    if (result_never_putin == "")
                    {
                        result_never_putin = item.ToString();
                    }
                    else
                    {
                        result_never_putin = result_never_putin + ",\n" + item;
            ...
            if (result_putin + result_never_putin != "")
            {
                return com.GetModuleFromJobjet(result,null, result_putin + result_never_putin);
            }
            else
            {
                return com.GetModuleFromJobjet(result,null, "没有模组开始老化");
            }
            //return Json(data,JsonRequestBehavior.AllowGet);
        }
        #endregion
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject CheckBarCodesList_B([System.Web.Http.FromBody]JObject data)
        {
            JArray result = new JArray();
            string OrderNum = data["OrderNum"].ToString();
            var BarCodesNumList = (JArray)data["BarCodesNumList"];
            foreach (var barcode in BarCodesNumList)
            {
                JObject item = new JObject();
                var barcodetostring = barcode.ToString();
                //一、检查此条码是否在此订单中（如果不存在，则返回条码所属的订单号，返回条码老化工序的状态，并跳出foreach循环）
                if (OrderNum == db.BarCodes.Where(c => c.BarCodesNum == barcodetostring).Select(c => c.OrderNum).FirstOrDefault())
                {


                    item.Add("barcode", barcode);//1.条码
                    item.Add("barcodeinorder", "Yes");//2.是否在订单中
                    item.Add("barcodeinotherorder", "");//3.在其他订单号中

                    //取出所有此条码的老化记录列表(包括已经开始老化未完成的、已经老化完成的)
                    var burnincount = db.Burn_in.Where(c => c.BarCodesNum == barcodetostring && (c.OldBarCodesNum == null || c.OldBarCodesNum == barcodetostring)).ToList();
                    //拿到重复数据中最后的一条数据
                    var distinBarcodelist = (from s in burnincount group s by s.BarCodesNum into g select new { id = g.Max(x => x.Id), BarCodesNum = g.Key, Normal = g.OrderByDescending(x => x.Id).Select(c => c.OQCCheckFinish).FirstOrDefault(), FT = g.OrderByDescending(x => x.Id).Select(x => x.OQCCheckFT).FirstOrDefault() }).ToList();

                    //二、检查此条码是否有老化记录,没有老化记录,则可以老化
                    if (burnincount.Count == 0)
                    {
                        item.Add("barcodebegintime", "");//4.开始时间
                        item.Add("barcodefinishtime", "");//5.完成时间
                        item.Add("barcodestatus", "未开始老化");//6.老化状态
                        item.Add("barcodecomfirm", "Yes");//7.可以进行老化
                    }
                    //三、条码已经有老化记录并只有一条
                    else if (burnincount.Count == 1)
                    {
                        item.Add("barcodebegintime", Convert.ToString(burnincount.FirstOrDefault().OQCCheckBT));//4.开始时间
                        if (burnincount.FirstOrDefault().OQCCheckFT != null && burnincount.FirstOrDefault().OQCCheckFinish == true)
                        {
                            //有一个记录，并且已经完成的
                            item.Add("barcodefinishtime", Convert.ToString(burnincount.FirstOrDefault().OQCCheckFT));//5.完成时间
                            item.Add("barcodestatus", "已经完成老化"); //6.老化状态
                            item.Add("barcodecomfirm", "No");//7.可以进行老化
                        }
                        else if (burnincount.FirstOrDefault().OQCCheckFT == null && burnincount.FirstOrDefault().OQCCheckFinish == false)
                        {
                            //有一个记录，没有完成时间的
                            item.Add("barcodefinishtime", "");//5.完成时间
                            item.Add("barcodestatus", "正在老化"); //6.老化状态
                            item.Add("barcodecomfirm", "No");//7.可以进行老化
                        }
                        else if (burnincount.FirstOrDefault().OQCCheckFT != null && burnincount.FirstOrDefault().OQCCheckFinish == false && burnincount.FirstOrDefault().Burn_in_OQCCheckAbnormal != "正常")
                        {
                            //有一个记录，已经完成老化，有异常，没有通过OQC
                            item.Add("barcodefinishtime", Convert.ToString(burnincount.FirstOrDefault().OQCCheckFT));//5.完成时间
                            item.Add("barcodestatus", "未通过老化"); //6.老化状态
                            item.Add("barcodecomfirm", "Yes");//7.可以进行老化
                        }
                    ...
                    {
                        int finishtrue = burnincount.Count(m => m.OQCCheckFinish == true);
                        //有一个完成记录
                        if (finishtrue >= 1)
                        {
                            var record = (from m in burnincount where m.OQCCheckFinish == true select m).ToList();
                            item.Add("barcodebegintime", Convert.ToString(record.FirstOrDefault().OQCCheckBT));//4.开始时间
                            item.Add("barcodefinishtime", Convert.ToString(record.FirstOrDefault().OQCCheckFT));//5.完成时间
                            item.Add("barcodestatus", "经过" + burnincount.Count() + "次老化，已经完成老化"); //6.老化状态
                            item.Add("barcodecomfirm", "No");//7.可以进行老化
                        }
                        //没有完成记录
                        else
                        {
                            //检查是否有正在进行老化的记录，有、无
                            if (burnincount.Count(m => m.OQCCheckBT != null && m.OQCCheckFT == null) >= 1)
                            {
                                var record = burnincount.Where(m => m.OQCCheckBT != null && m.OQCCheckFT == null).ToList().FirstOrDefault();
                                item.Add("barcodebegintime", Convert.ToString(record.OQCCheckBT));//4.开始时间
                                item.Add("barcodefinishtime", "");//5.完成时间
                                item.Add("barcodestatus", "第" + burnincount.Count(m => m.OQCCheckBT != null && m.OQCCheckFT == null) + "次老化，正在老化"); //6.老化状态
                                item.Add("barcodecomfirm", "No");//7.可以进行老化
                            }
                            else
                            {
                                item.Add("barcodebegintime", "");//4.开始时间
                                item.Add("barcodefinishtime", "");//5.完成时间
                                item.Add("barcodestatus", "经过" + burnincount.Count(m => m.OQCCheckBT != null && m.OQCCheckFT != null) + "次老化，但未通过老化"); //6.老化状态
                                item.Add("barcodecomfirm", "Yes");//7.可以进行老化
                            }
                        }
                    }
                ...
                {
                    var AnotherOrderNum = db.BarCodes.Where(c => c.BarCodesNum == barcodetostring).Select(c => c.OrderNum).FirstOrDefault();
                    item.Add("barcode", barcode);//1.条码
                    item.Add("barcodeinorder", "No"); //2.是否在订单中
                    item.Add("barcodeinotherorder", AnotherOrderNum);//3.在其他订单号中
                    item.Add("barcodebegintime", "");//4.开始时间
                    item.Add("barcodefinishtime", "");//5.完成时间
                    item.Add("barcodestatus", ""); //6.老化状态
                    item.Add("barcodecomfirm", "");//7.可以进行老化
                }
                result.Add(item);
            }
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject CheckBarCodesList_F([System.Web.Http.FromBody]JObject data)
        {
            ...
            foreach (var item in BarCodesNumList)
            {
                ...
                if (barcodeCountin == 1)
                {
                    ...
                    //此条码在OrderNum订单中
                    barcodeitem.Add("barcodelist", barcode);//1.条码
                    barcodeitem.Add("barcodeinorder", "Yes");//2.是否在订单中
                    barcodeitem.Add("barcodeinotherorder", "");//3.在其他订单号中
                    ...
                    if (burninrecords.Count() == 0)
                    {
                        ...
                        barcodeitem.Add("barcodebegintime", "");//4.开始时间
                        barcodeitem.Add("barcodefinishtime", "");//5.完成时间
                        barcodeitem.Add("barcodestatus", "未开始老化");//6.老化状态
                        barcodeitem.Add("barcodecomfirm", "No");//7.可以进行老化完成
                    }
                    //一个记录
                    if (burninrecords.Count() == 1)
                    {
                        barcodeitem.Add("barcodebegintime", Convert.ToString(burninrecords.FirstOrDefault().OQCCheckBT));//4.开始时间
                        if (burninrecords.FirstOrDefault().OQCCheckFT != null && burninrecords.FirstOrDefault().OQCCheckFinish == true)
                        {
                            //有一个记录，并且已经完成的
                            barcodeitem.Add("barcodefinishtime", Convert.ToString(burninrecords.FirstOrDefault().OQCCheckFT));//5.完成时间
                            barcodeitem.Add("barcodestatus", "已经完成老化"); //6.老化状态
                            barcodeitem.Add("barcodecomfirm", "No");//7.可以进行老化完成
                        }
                        else if (burninrecords.FirstOrDefault().OQCCheckFT == null && burninrecords.FirstOrDefault().OQCCheckFinish == false)
                        {
                            //有一个记录，没有完成时间的
                            barcodeitem.Add("barcodefinishtime", "");//5.完成时间
                            barcodeitem.Add("barcodestatus", "正在老化"); //6.老化状态
                            barcodeitem.Add("barcodecomfirm", "Yes");//7.可以进行老化完成
                        }
                        else if (burninrecords.FirstOrDefault().OQCCheckFT != null && burninrecords.FirstOrDefault().OQCCheckFinish == false && burninrecords.FirstOrDefault().Burn_in_OQCCheckAbnormal != "正常")
                        {
                            //有一个记录，已经完成老化，有异常，没有通过OQC
                            barcodeitem.Add("barcodefinishtime", Convert.ToString(burninrecords.FirstOrDefault().OQCCheckFT));//5.完成时间
                            barcodeitem.Add("barcodestatus", "未通过老化"); //6.老化状态
                            barcodeitem.Add("barcodecomfirm", "No");//7.可以进行老化完成
                        }
                    }
                    //超过一个记录
                    if (burninrecords.Count() > 1)
                    {
                        ...
                        {
                            var record = (from m in burninrecords where m.OQCCheckFinish == true select m).ToList();
                            barcodeitem.Add("barcodebegintime", Convert.ToString(record.FirstOrDefault().OQCCheckBT));//4.开始时间
                            barcodeitem.Add("barcodefinishtime", Convert.ToString(record.FirstOrDefault().OQCCheckFT));//5.完成时间
                            barcodeitem.Add("barcodestatus", "经过" + burninrecords.Count() + "次老化，已经完成老化"); //6.老化状态
                            barcodeitem.Add("barcodecomfirm", "No");//7.可以进行老化
                        }
                        //没有完成记录
                        else if (finisttrue == 0)
                        {
                            ...
                            {
                                var record = burninrecords.Where(m => m.OQCCheckFT == null).ToList().FirstOrDefault();
                                barcodeitem.Add("barcodebegintime", Convert.ToString(record.OQCCheckBT));//4.开始时间
                                barcodeitem.Add("barcodefinishtime", "");//5.完成时间
                                barcodeitem.Add("barcodestatus", "第" + burninrecords.Count() + "次老化，正在老化"); //6.老化状态
                                barcodeitem.Add("barcodecomfirm", "Yes");//7.可以进行老化
                            }
                            else
                            {
                                barcodeitem.Add("barcodebegintime", "");//4.开始时间
                                barcodeitem.Add("barcodefinishtime", "");//5.完成时间
                                barcodeitem.Add("barcodestatus", "经过" + burninrecords.Count() + "次老化，但未通过老化"); //6.老化状态
                                barcodeitem.Add("barcodecomfirm", "No");//7.可以进行老化
                            }
                        }
                    }
                ...
                {
                    var AnotherOrderNum = db.BarCodes.Where(c => c.BarCodesNum == barcode).Select(c => c.OrderNum).FirstOrDefault();
                    barcodeitem.Add("barcodelist", barcode);//1.条码
                    barcodeitem.Add("barcodeinorder", "No"); //2.是否在订单中
                    barcodeitem.Add("barcodeinotherorder", AnotherOrderNum);//3.在其他订单号中
                    barcodeitem.Add("barcodebegintime", "");//4.开始时间
                    barcodeitem.Add("barcodefinishtime", "");//5.完成时间
                    barcodeitem.Add("barcodestatus", ""); //6.老化状态
                    barcodeitem.Add("barcodecomfirm", "");//7.可以进行老化
                    nuobarcode.Add("");
        ...
        [HttpPost]
        [ApiAuthorize]
        public void Burn_in_Batch_Finish([System.Web.Http.FromBody]JObject data)
        {
            string OrderNum = data["OrderNum"].ToString();
            string UserName = data["UserName"].ToString();
            JArray BarCodesNumList = (JArray)data["BarCodesNumList"];
            ...
            foreach (var barcode in BarCodesNumList)
            {
                string item = barcode.ToString();
                var burn_in = (from x in burn_in_List where (x.BarCodesNum == item && x.OQCCheckFT == null) select x).FirstOrDefault();
                burn_in.OQCCheckFT = DateTime.Now;
                //计算时间
                var time = comapi.CalculateTimespan(burn_in.OQCCheckBT.Value, burn_in.OQCCheckFT.Value);
                burn_in.OQCCheckTime = TimeSpan.Parse(time["Time"].ToString());
                burn_in.OQCCheckDate = int.Parse(time["Date"].ToString());
                burn_in.OQCCheckTimeSpan = time["TimeSpan"].ToString();
        ...
        [HttpPost]
        [ApiAuthorize]
        public JObject Burn_inChecklist(string orderNum)
        {
            JObject stationResult = new JObject();//输出结果JObject
            if (!String.IsNullOrEmpty(orderNum))
            {
                //调出订单对应全部记录      
                var AllBurn_in = db.Burn_in.Where(c => c.OrderNum == orderNum && (c.OldOrderNum == null || c.OldOrderNum == orderNum)).Select(c => new Common_ApiController.Temp { id = c.Id, BarCodesNum = c.BarCodesNum, Finsh = c.OQCCheckFinish, FinshTime = c.OQCCheckFT }).ToList();
                stationResult = comapi.SectionCehckList(orderNum, AllBurn_in);
