if(!jQuery) throw new Error("Bootstrap requires jQuery"); + function(n) {
	...
	t.prototype.toggle = function() {
		var n = this.$element.closest('[data-toggle="buttons"]'),
			t;
		n.length && (t = this.$element.find("input").prop("checked", !this.$element.hasClass("active")).trigger("change"), t.prop("type") === "radio" && n.find(".active").removeClass("active"));
	...
	t.prototype.checkPosition = function() {
		...
		if(this.$element.is(":visible")) {
			var s = n(document).height(),
				...
				f = r.top,
				u = r.bottom;
			(typeof r != "object" && (u = f = r), typeof f == "function" && (f = r.top()), typeof u == "function" && (u = r.bottom()), i = this.unpin != null && e + this.unpin <= o.top ? !1 : u != null && o.top + this.$element.height() >= s - u ? "bottom" : f != null && e <= f ? "top" : !1, this.affixed !== i) && (this.unpin && this.$element.css("top", ""), this.affixed = i, this.unpin = i == "bottom" ? o.top - e : null, this.$element.removeClass(t.RESET).addClass("affix" + (i ? "-" + i : "")), i == "bottom" && this.$element.offset({
...
function(n) {
	...
	t.prototype = {
		init: function(t, i) {
			this.$element = n(t);
			var r = i && i.bootstrapMajorVersion ? i.bootstrapMajorVersion : n.fn.bootstrapPaginator.defaults.bootstrapMajorVersion,
				u = this.$element.attr("id");
			if(r !== 2 || this.$element.is("div")) {
				if(r > 2 && !this.$element.is("ul")) throw "in Bootstrap version 3 the pagination root item must be an ul element.";
			} else throw "in Bootstrap version 2 the pagination must be a div element. Or if you are using Bootstrap pagination 3. Please specify it in bootstrapMajorVersion in the option";
			this.currentPage = 1;
			this.lastPage = 1;
			this.setOptions(i);
			this.initialized = !0
		...
		setOptions: function(t) {
			this.options = n.extend({}, this.options || n.fn.bootstrapPaginator.defaults, t);
			this.totalPages = parseInt(this.options.totalPages, 10);
			this.numberOfPages = parseInt(this.options.numberOfPages, 10);
			t && typeof t.currentPage != "undefined" && this.setCurrentPage(t.currentPage);
			this.listen();
			this.render();
			this.initialized || this.lastPage === this.currentPage || this.$element.trigger("page-changed", [this.lastPage, this.currentPage])
		},
		listen: function() {
			if(this.$element.off("page-clicked"), this.$element.off("page-changed"), typeof this.options.onPageClicked == "function" && this.$element.bind("page-clicked", this.options.onPageClicked), typeof this.options.onPageChanged == "function") this.$element.on("page-changed", this.options.onPageChanged);
		...
		showNext: function() {
			var n = this.getPages();
			n.next && this.show(n.next)
		},
		showPrevious: function() {
			var n = this.getPages();
			n.prev && this.show(n.prev)
		},
		showFirst: function() {
			var n = this.getPages();
			n.first && this.show(n.first)
		},
		showLast: function() {
			var n = this.getPages();
			n.last && this.show(n.last)
		},
		onPageItemClicked: function(n) {
			var t = n.data.type,
				i = n.data.page;
			this.$element.trigger("page-clicked", [n, t, i])
		},
		onPageClicked: function(t, i, r, u) {
			...
			switch(r) {
				...
				case "last":
					...
					break;
				case "page":
					f.bootstrapPaginator("show", u)
			}
		},
		render: function() {
			var c = this.getValueFromOption(this.options.containerClass, this.$element),
				h = this.options.size || "normal",
				l = this.options.alignment || "left",
				t = this.getPages(),
				i = this.options.bootstrapMajorVersion === 2 ? n("<ul><\/ul>") : this.$element,
				a = this.options.bootstrapMajorVersion === 2 ? this.getValueFromOption(this.options.listContainerClass, i) : null,
				u = null,
				f = null,
				e = null,
				o = null,
			...
			switch(h.toLowerCase()) {
				...
				case "small":
				case "mini":
					this.$element.addClass(n.fn.bootstrapPaginator.sizeArray[this.options.bootstrapMajorVersion][h.toLowerCase()])
			}
			if(this.options.bootstrapMajorVersion === 2) switch(l.toLowerCase()) {
				case "center":
			...
			for(this.$element.addClass(c), this.$element.empty(), this.options.bootstrapMajorVersion === 2 && (this.$element.append(i), i.addClass(a)), this.pageRef = [], t.first && (u = this.buildPageItem("first", t.first), u && i.append(u)), t.prev && (f = this.buildPageItem("prev", t.prev), f && i.append(f)), r = 0; r < t.length; r = r + 1) s = this.buildPageItem("page", t[r]), s && i.append(s);
			t.next && (e = this.buildPageItem("next", t.next), e && i.append(e));
			t.last && (o = this.buildPageItem("last", t.last), o && i.append(o))
		},
		buildPageItem: function(t, i) {
			var e = n("<li><\/li>"),
				f = n("<a><\/a>"),
				u = "",
				r = "",
				s = this.options.itemContainerClass(t, i, this.currentPage),
				h = this.getValueFromOption(this.options.itemContentClass, t, i, this.currentPage),
				o = null;
			switch(t) {
				case "first":
					if(!this.getValueFromOption(this.options.shouldShowPage, t, i, this.currentPage)) return;
					u = this.options.itemTexts(t, i, this.currentPage);
					r = this.options.tooltipTitles(t, i, this.currentPage);
					break;
				case "last":
					if(!this.getValueFromOption(this.options.shouldShowPage, t, i, this.currentPage)) return;
					u = this.options.itemTexts(t, i, this.currentPage);
			...
			e.addClass(s).append(f);
			f.addClass(h).html(u).on("click", null, {
				type: t,
				page: i
			}, n.proxy(this.onPageItemClicked, this));
			return this.options.pageUrl && f.attr("href", this.getValueFromOption(this.options.pageUrl, t, i, this.currentPage)), this.options.useBootstrapTooltip ? (o = n.extend({}, this.options.bootstrapTooltipOptions, {
				title: r
			}), f.tooltip(o)) : f.attr("title", r), e
		},
		setCurrentPage: function(n) {
			if(n > this.totalPages || n < 1) throw "Page out of range";
			this.lastPage = this.currentPage;
			this.currentPage = parseInt(n, 10)
		},
		getPages: function() {
			var t = this.totalPages,
				r = this.currentPage % this.numberOfPages == 0 ? (parseInt(this.currentPage / this.numberOfPages, 10) - 1) * this.numberOfPages + 1 : parseInt(this.currentPage / this.numberOfPages, 10) * this.numberOfPages + 1,
				n = [],
				i = 0,
				u = 0;
			for(r = r < 1 ? 1 : r, i = r, u = 0; u < this.numberOfPages && i <= t; i = i + 1, u = u + 1) n.push(i);
			return n.first = 1, n.prev = this.currentPage > 1 ? this.currentPage - 1 : 1, n.next = this.currentPage < t ? this.currentPage + 1 : t, n.last = t, n.current = this.currentPage, n.total = t, n.numberOfPages = this.options.numberOfPages, n
		},
		getValueFromOption: function(n) {
			var t = Array.prototype.slice.call(arguments, 1);
			return typeof n == "function" ? n.apply(this, t) : n
		}
	};
	i = n.fn.bootstrapPaginator;
	n.fn.bootstrapPaginator = function(i) {
		var u = arguments,
			r = null;
		return n(this).each(function(f, e) {
			var s = n(e),
				o = s.data("bootstrapPaginator"),
				h = typeof i != "object" ? null : i;
			if(!o) {
				o = new t(this, h);
				s = n(o.$element);
				s.data("bootstrapPaginator", o);
				return
			}
			if(typeof i == "string")
				if(o[i]) r = o[i].apply(o, Array.prototype.slice.call(u, 1));
				else throw "Method " + i + " does not exist";
			else r = o.setOptions(i)
		}), r
	};
	n.fn.bootstrapPaginator.sizeArray = {
		"2": {
	...
	n.fn.bootstrapPaginator.defaults = {
		containerClass: "",
		size: "normal",
		alignment: "left",
		bootstrapMajorVersion: 2,
		listContainerClass: "",
		itemContainerClass: function(n, t, i) {
			return t === i ? "active" : ""
		},
	...
	n.fn.bootstrapPaginator.Constructor = t
}(window.jQuery);
