    ...
    public class SMTBroadcaster
    {
        ...
        private void SMTBroadcastShape(object state)
        {
            ...
            using (var db = new ApplicationDbContext())
            {
                ...
                //var SMT_ProductionPlans_by_Group_temp = SMT_ProductionPlans.Join(SMT_ProductionDatas, c => c.OrderNum, a => a.OrderNum, (c, a) => new { c.OrderNum, c.LineNum, c.JobContent, c.PlanProductionDate, a.ProductionDate, a.NormalCount }).OrderByDescending(a => a.ProductionDate).Select(b => new { b.OrderNum, b.LineNum, b.JobContent, b.ProductionDate, b.NormalCount, b.PlanProductionDate }).ToList();
                //var SMT_ProductionPlans_by_Group_temp0 = SMT_ProductionPlans_by_Group_temp.OrderByDescending(c => c.NormalCount).Select(c => new { c.LineNum, c.OrderNum, c.JobContent, c.ProductionDate, c.PlanProductionDate });
                //var SMT_ProductionPlans_by_Group_temp1 = SMT_ProductionPlans_by_Group_temp0.OrderByDescending(c => c.PlanProductionDate).Select(c => new { c.LineNum, c.OrderNum, c.JobContent }).Distinct().ToList();


                //取出计划订单清单(去重)
                //var SMT_ProductionPlans_list = SMT_ProductionPlans_by_Group_temp1.Select(c =>c.OrderNum).Distinct();
